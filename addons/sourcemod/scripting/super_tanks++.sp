			...
			if (bIsSurvivor(victim))
			{
				if (bIsBotInfected(attacker) && bIsTank(attacker) && damagetype != 2)
				{
					switch (g_iTankType[attacker])
					{
						case 1:
						{
							if (StrEqual(sClassname, "weapon_tank_rock", false) || StrEqual(sClassname, "weapon_tank_rock", false))
							{
								vAcidHit(victim);
							}
						}
	...
	if (GetRandomInt(1, g_cvSTAmmoChance.IntValue) == 1 && bIsSurvivor(client) && GetPlayerWeaponSlot(client, 0) > 0)
	{
		...
		if (IsValidEntity(iActiveWeapon))
		{
			...
			else
			{
				...
				{
					SetEntData(client, (FindDataMapInfo(client, "m_iAmmo") + 8), g_cvSTAmmoCount.IntValue);
				}
				...
				{
					SetEntData(client, (FindDataMapInfo(client, "m_iAmmo") + 24), g_cvSTAmmoCount.IntValue);
				}
			}
		}
...
public Action tTimerDeleteEntity(Handle timer, any particle)
{
