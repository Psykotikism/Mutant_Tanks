	...
	for (int iIndex = ST_MinType(); iIndex <= ST_MaxType(); iIndex++)
	{
		...
		if (kvSuperTanks.JumpToKey(sName))
		{
			if (main)
			{
				...
				g_iPyroChance[iIndex] = kvSuperTanks.GetNum("Pyro Ability/Pyro Chance", 4);
				g_iPyroChance[iIndex] = iClamp(g_iPyroChance[iIndex], 1, 9999999999);
				...
				g_iPyroRangeChance[iIndex] = kvSuperTanks.GetNum("Pyro Ability/Pyro Range Chance", 16);
				g_iPyroRangeChance[iIndex] = iClamp(g_iPyroRangeChance[iIndex], 1, 9999999999);
			}
			else
			{
				...
				g_iPyroChance2[iIndex] = kvSuperTanks.GetNum("Pyro Ability/Pyro Chance", g_iPyroChance[iIndex]);
				g_iPyroChance2[iIndex] = iClamp(g_iPyroChance2[iIndex], 1, 9999999999);
				...
				g_iPyroRangeChance2[iIndex] = kvSuperTanks.GetNum("Pyro Ability/Pyro Range Chance", g_iPyroRangeChance[iIndex]);
				g_iPyroRangeChance2[iIndex] = iClamp(g_iPyroRangeChance2[iIndex], 1, 9999999999);
			}
...
public void ST_Ability(int tank)
{
	...
	{
		int iPyroRangeChance = !g_bTankConfig[ST_TankType(tank)] ? g_iPyroChance[ST_TankType(tank)] : g_iPyroChance2[ST_TankType(tank)];
		...
		for (int iSurvivor = 1; iSurvivor <= MaxClients; iSurvivor++)
		{
			if (bIsSurvivor(iSurvivor))
			{
				...
				{
					vPyroHit(iSurvivor, tank, iPyroRangeChance, iPyroAbility(tank), 2, "3");
				}
			}
		}
	}
...
static void vPyroHit(int survivor, int tank, int chance, int enabled, int message, const char[] mode)
{
	if ((enabled == 1 || enabled == 3) && GetRandomInt(1, chance) == 1 && bIsSurvivor(survivor) && !g_bPyro[tank])
	{
...
static int iPyroChance(int tank)
{
	return !g_bTankConfig[ST_TankType(tank)] ? g_iPyroChance[ST_TankType(tank)] : g_iPyroChance2[ST_TankType(tank)];
}
