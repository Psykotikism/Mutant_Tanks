	...
	if (bIsTank(client))
	{
		...
		for (int iSurvivor = 1; iSurvivor <= MaxClients; iSurvivor++)
		{
			if (bIsSurvivor(iSurvivor))
			{
				...
				{
					vShakeHit(iSurvivor, client, iShakeAbility);
				}
			}
		}
	}
...
{
	int iShakeChance = !g_bTankConfig[ST_TankType(owner)] ? g_iShakeChance[ST_TankType(owner)] : g_iShakeChance2[ST_TankType(owner)];
	if (enabled == 1 && GetRandomInt(1, iShakeChance) == 1 && bIsSurvivor(client) && !g_bShake[client])
	{
