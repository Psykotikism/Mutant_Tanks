...
int g_iAccessFlags[ST_MAXTYPES + 1], g_iAccessFlags2[MAXPLAYERS + 1], g_iHumanAbility[ST_MAXTYPES + 1], g_iHumanAmmo[ST_MAXTYPES + 1], g_iIceAbility[ST_MAXTYPES + 1], g_iIceCount[MAXPLAYERS + 1], g_iIceEffect[ST_MAXTYPES + 1], g_iIceHit[ST_MAXTYPES + 1], g_iIceHitMode[ST_MAXTYPES + 1], g_iIceMessage[ST_MAXTYPES + 1], g_iIceOwner[MAXPLAYERS + 1], g_iImmunityFlags[ST_MAXTYPES + 1], g_iImmunityFlags2[MAXPLAYERS + 1];
...
public Action OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype)
{
	if (ST_IsCorePluginEnabled() && bIsValidClient(victim, ST_CHECK_INDEX|ST_CHECK_INGAME|ST_CHECK_ALIVE|ST_CHECK_KICKQUEUE) && damage > 0.0)
	{
		...
		if (ST_IsTankSupported(attacker) && bIsCloneAllowed(attacker, g_bCloneInstalled) && (g_iIceHitMode[ST_GetTankType(attacker)] == 0 || g_iIceHitMode[ST_GetTankType(attacker)] == 1) && bIsSurvivor(victim))
		{
		...
		else if (ST_IsTankSupported(victim) && bIsCloneAllowed(victim, g_bCloneInstalled) && (g_iIceHitMode[ST_GetTankType(victim)] == 0 || g_iIceHitMode[ST_GetTankType(victim)] == 2) && bIsSurvivor(attacker))
		{
...
public Action tTimerStopIce(Handle timer, DataPack pack)
{
	...
	if (ST_IsTankSupported(iTank, ST_CHECK_FAKECLIENT) && (ST_HasAdminAccess(iTank) || bHasAdminAccess(iTank)) && g_iHumanAbility[ST_GetTankType(iTank)] == 1 && (iMessage & ST_MESSAGE_RANGE) && !g_bIce3[iTank])
	{
		g_bIce3[iTank] = true;
