...
int g_iAccessFlags[ST_MAXTYPES + 1], g_iAccessFlags2[MAXPLAYERS + 1], g_iHumanAbility[ST_MAXTYPES + 1], g_iHumanAmmo[ST_MAXTYPES + 1], g_iImmunityFlags[ST_MAXTYPES + 1], g_iImmunityFlags2[MAXPLAYERS + 1], g_iSlowAbility[ST_MAXTYPES + 1], g_iSlowCount[MAXPLAYERS + 1], g_iSlowEffect[ST_MAXTYPES + 1], g_iSlowHit[ST_MAXTYPES + 1], g_iSlowHitMode[ST_MAXTYPES + 1], g_iSlowMessage[ST_MAXTYPES + 1], g_iSlowOwner[MAXPLAYERS + 1];
...
public void ST_OnConfigsLoaded(const char[] subsection, const char[] key, const char[] value, int type, int admin)
{
	...
	if (type > 0)
	{
		ST_FindAbility(type, 55, bHasAbilities(subsection, "slowability", "slow ability", "slow_ability", "slow"));
...
public Action tTimerStopSlow(Handle timer, DataPack pack)
{
	...
	if (ST_IsTankSupported(iTank, ST_CHECK_FAKECLIENT) && (ST_HasAdminAccess(iTank) || bHasAdminAccess(iTank)) && g_iHumanAbility[ST_GetTankType(iTank)] == 1 && (iMessage & ST_MESSAGE_RANGE) && !g_bSlow3[iTank])
	{
		g_bSlow3[iTank] = true;
