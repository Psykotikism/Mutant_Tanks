...
{
	if (ST_TankAllowed(tank) && (!ST_TankAllowed(tank, "5") || iHumanAbility(tank) == 0) && ST_CloneAllowed(tank, g_bCloneInstalled) && iRecoilAbility(tank) == 1)
	{
		vRecoilAbility(tank);
	}
}
	...
	if (enabled == 1 && bIsSurvivor(survivor))
	{
		if (g_iRecoilCount[tank] < iHumanAmmo(tank) && iHumanAmmo(tank) > 0)
		{
			if (GetRandomFloat(0.1, 100.0) <= chance && !g_bRecoil[survivor])
			{
				...
				if (ST_TankAllowed(tank, "5") && iHumanAbility(tank) == 1 && StrEqual(mode, "3") && !g_bRecoil2[tank])
				{
			...
			{
				if (ST_TankAllowed(tank, "5") && iHumanAbility(tank) == 1 && !g_bRecoil4[tank])
				{
		...
		else if (ST_TankAllowed(tank, "5") && iHumanAbility(tank) == 1 && !g_bRecoil5[tank])
		{
...
{
	return !g_bTankConfig[ST_TankType(tank)] ? g_flRecoilDuration[ST_TankType(tank)] : g_flRecoilDuration2[ST_TankType(tank)];
}
...
public Action tTimerStopRecoil(Handle timer, DataPack pack)
{
	...
	if (ST_TankAllowed(iTank, "5") && iHumanAbility(iTank) == 1 && StrContains(sMessage, "2") != -1 && !g_bRecoil3[iTank])
	{
...
public Action tTimerResetCooldown(Handle timer, int userid)
{
	...
	if (!ST_TankAllowed(iTank, "02345") || !ST_CloneAllowed(iTank, g_bCloneInstalled) || !g_bRecoil3[iTank])
	{
		g_bRecoil3[iTank] = false;
