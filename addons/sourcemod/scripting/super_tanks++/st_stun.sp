...
bool g_bLateLoad, g_bPluginEnabled, g_bStun[MAXPLAYERS + 1], g_bTankConfig[ST_MAXTYPES + 1];
...
{
	int iStunAbility = !g_bTankConfig[ST_TankType(client)] ? g_iStunAbility[ST_TankType(client)] : g_iStunAbility2[ST_TankType(client)];
	if (g_bPluginEnabled && ST_TankAllowed(client) && iStunAbility == 1)
	{
		vRemoveStun(client);
	}
}
...
{
	if (g_bPluginEnabled && enabled == 1 && GetRandomInt(1, chance) == 1 && bIsSurvivor(client) && !g_bStun[client])
	{
		g_bStun[client] = true;
