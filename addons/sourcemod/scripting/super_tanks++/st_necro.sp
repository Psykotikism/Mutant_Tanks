...
{
	if (ST_IsTankSupported(tank) && (!ST_IsTankSupported(tank, ST_CHECK_FAKECLIENT) || iHumanAbility(tank) == 0) && ST_IsCloneSupported(tank, g_bCloneInstalled) && iNecroAbility(tank) == 1 && !g_bNecro[tank])
	{
		vNecroAbility(tank);
	}
}
...
static void vNecroAbility(int tank)
{
	if (g_iNecroCount[tank] < iHumanAmmo(tank) && iHumanAmmo(tank) > 0)
	{
		if (GetRandomFloat(0.1, 100.0) <= flNecroChance(tank))
		{
			...
			if (ST_IsTankSupported(tank, ST_CHECK_FAKECLIENT) && iHumanAbility(tank) == 1 && !g_bNecro2[tank])
			{
				g_iNecroCount[tank]++;
...
public Action tTimerStopNecro(Handle timer, int userid)
{
	...
	if (ST_IsTankSupported(iTank, ST_CHECK_FAKECLIENT) && iHumanAbility(iTank) == 1 && iHumanMode(iTank) == 0 && !g_bNecro2[iTank])
	{
		vReset2(iTank);
	}
...
{
	int iTank = GetClientOfUserId(userid);
	if (!ST_IsTankSupported(iTank, ST_CHECK_INDEX|ST_CHECK_INGAME|ST_CHECK_ALIVE|ST_CHECK_KICKQUEUE|ST_CHECK_FAKECLIENT) || !ST_IsCloneSupported(iTank, g_bCloneInstalled) || !g_bNecro2[iTank])
	{
