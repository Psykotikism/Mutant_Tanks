...
float g_flHumanCooldown[MT_MAXTYPES + 1], g_flNullifyChance[MT_MAXTYPES + 1], g_flNullifyDuration[MT_MAXTYPES + 1], g_flNullifyRange[MT_MAXTYPES + 1], g_flNullifyRangeChance[MT_MAXTYPES + 1];

int g_iAccessFlags[MT_MAXTYPES + 1], g_iAccessFlags2[MAXPLAYERS + 1], g_iHumanAbility[MT_MAXTYPES + 1], g_iHumanAmmo[MT_MAXTYPES + 1], g_iImmunityFlags[MT_MAXTYPES + 1], g_iImmunityFlags2[MAXPLAYERS + 1], g_iNullifyAbility[MT_MAXTYPES + 1], g_iNullifyCount[MAXPLAYERS + 1], g_iNullifyEffect[MT_MAXTYPES + 1], g_iNullifyHit[MT_MAXTYPES + 1], g_iNullifyHitMode[MT_MAXTYPES + 1], g_iNullifyMessage[MT_MAXTYPES + 1], g_iNullifyOwner[MAXPLAYERS + 1];
...
public int iNullifyMenuHandler(Menu menu, MenuAction action, int param1, int param2)
{
	switch (action)
	{
		...
		case MenuAction_Select:
		{
			switch (param2)
			{
				case 0: MT_PrintToChat(param1, "%s %t", MT_TAG3, g_iNullifyAbility[MT_GetTankType(param1)] == 0 ? "AbilityStatus1" : "AbilityStatus2");
				case 1: MT_PrintToChat(param1, "%s %t", MT_TAG3, "AbilityAmmo", g_iHumanAmmo[MT_GetTankType(param1)] - g_iNullifyCount[param1], g_iHumanAmmo[MT_GetTankType(param1)]);
				...
				case 5: MT_PrintToChat(param1, "%s %t", MT_TAG3, "AbilityDuration", g_flNullifyDuration[MT_GetTankType(param1)]);
...
public Action OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype)
{
	if (MT_IsCorePluginEnabled() && bIsValidClient(victim, MT_CHECK_INDEX|MT_CHECK_INGAME|MT_CHECK_ALIVE|MT_CHECK_KICKQUEUE) && damage > 0.0)
	{
		...
		if (MT_IsTankSupported(attacker) && bIsCloneAllowed(attacker, g_bCloneInstalled) && (g_iNullifyHitMode[MT_GetTankType(attacker)] == 0 || g_iNullifyHitMode[MT_GetTankType(attacker)] == 1) && bIsSurvivor(victim))
		{
			...
			{
				vNullifyHit(victim, attacker, g_flNullifyChance[MT_GetTankType(attacker)], g_iNullifyHit[MT_GetTankType(attacker)], MT_MESSAGE_MELEE, MT_ATTACK_CLAW);
			}
		}
		else if (MT_IsTankSupported(victim) && bIsCloneAllowed(victim, g_bCloneInstalled) && bIsSurvivor(attacker))
		{
			if ((g_iNullifyHitMode[MT_GetTankType(victim)] == 0 || g_iNullifyHitMode[MT_GetTankType(victim)] == 2) && StrEqual(sClassname, "weapon_melee"))
			{
				if ((MT_HasAdminAccess(victim) || bHasAdminAccess(victim)) && !MT_IsAdminImmune(attacker, victim) && !bIsAdminImmune(attacker, victim))
				{
					vNullifyHit(attacker, victim, g_flNullifyChance[MT_GetTankType(victim)], g_iNullifyHit[MT_GetTankType(victim)], MT_MESSAGE_MELEE, MT_ATTACK_MELEE);
				}
			}
...
public void MT_OnConfigsLoaded(const char[] subsection, const char[] key, const char[] value, int type, int admin)
{
	...
	if (type > 0)
	{
		MT_FindAbility(type, 39, bHasAbilities(subsection, "nullifyability", "nullify ability", "nullify_ability", "nullify"));
...
public void MT_OnAbilityActivated(int tank)
{
	...
	if (MT_IsTankSupported(tank) && (!MT_IsTankSupported(tank, MT_CHECK_FAKECLIENT) || g_iHumanAbility[MT_GetTankType(tank)] == 0) && bIsCloneAllowed(tank, g_bCloneInstalled) && g_iNullifyAbility[MT_GetTankType(tank)] == 1)
	{
...
public void MT_OnButtonPressed(int tank, int button)
{
	if (MT_IsTankSupported(tank, MT_CHECK_INDEX|MT_CHECK_INGAME|MT_CHECK_ALIVE|MT_CHECK_KICKQUEUE|MT_CHECK_FAKECLIENT) && bIsCloneAllowed(tank, g_bCloneInstalled))
	{
		...
		{
			if (g_iNullifyAbility[MT_GetTankType(tank)] == 1 && g_iHumanAbility[MT_GetTankType(tank)] == 1)
			{
...
static void vNullifyAbility(int tank)
{
	...
	if (g_iNullifyCount[tank] < g_iHumanAmmo[MT_GetTankType(tank)] && g_iHumanAmmo[MT_GetTankType(tank)] > 0)
	{
		...
		for (int iSurvivor = 1; iSurvivor <= MaxClients; iSurvivor++)
		{
			if (bIsSurvivor(iSurvivor, MT_CHECK_INGAME|MT_CHECK_ALIVE|MT_CHECK_KICKQUEUE) && !MT_IsAdminImmune(iSurvivor, tank) && !bIsAdminImmune(iSurvivor, tank))
			{
				...
				if (flDistance <= g_flNullifyRange[MT_GetTankType(tank)])
				{
					vNullifyHit(iSurvivor, tank, g_flNullifyRangeChance[MT_GetTankType(tank)], g_iNullifyAbility[MT_GetTankType(tank)], MT_MESSAGE_RANGE, MT_ATTACK_RANGE);
	...
	if (enabled == 1 && bIsSurvivor(survivor))
	{
		if (g_iNullifyCount[tank] < g_iHumanAmmo[MT_GetTankType(tank)] && g_iHumanAmmo[MT_GetTankType(tank)] > 0)
		{
			if (GetRandomFloat(0.1, 100.0) <= chance && !g_bNullify[survivor])
			{
				...
				if (MT_IsTankSupported(tank, MT_CHECK_FAKECLIENT) && g_iHumanAbility[MT_GetTankType(tank)] == 1 && (flags & MT_ATTACK_RANGE) && !g_bNullify2[tank])
				{
					g_bNullify2[tank] = true;
					g_iNullifyCount[tank]++;

					MT_PrintToChat(tank, "%s %t", MT_TAG3, "NullifyHuman", g_iNullifyCount[tank], g_iHumanAmmo[MT_GetTankType(tank)]);
				}

				DataPack dpStopNullify;
				CreateDataTimer(g_flNullifyDuration[MT_GetTankType(tank)], tTimerStopNullify, dpStopNullify, TIMER_FLAG_NO_MAPCHANGE);
				...
				vEffect(survivor, tank, g_iNullifyEffect[MT_GetTankType(tank)], flags);

				if (g_iNullifyMessage[MT_GetTankType(tank)] & messages)
				{
					char sTankName[33];
			...
			else if ((flags & MT_ATTACK_RANGE) && !g_bNullify2[tank])
			{
				if (MT_IsTankSupported(tank, MT_CHECK_FAKECLIENT) && g_iHumanAbility[MT_GetTankType(tank)] == 1 && !g_bNullify4[tank])
				{
		...
		else if (MT_IsTankSupported(tank, MT_CHECK_FAKECLIENT) && g_iHumanAbility[MT_GetTankType(tank)] == 1 && !g_bNullify5[tank])
		{
...
static void vRemoveNullify(int tank)
{
	for (int iSurvivor = 1; iSurvivor <= MaxClients; iSurvivor++)
	{
		if (bIsSurvivor(iSurvivor, MT_CHECK_INGAME|MT_CHECK_KICKQUEUE) && g_bNullify[iSurvivor] && g_iNullifyOwner[iSurvivor] == tank)
		{
...
public Action tTimerStopNullify(Handle timer, DataPack pack)
{
	...
	if (MT_IsTankSupported(iTank, MT_CHECK_FAKECLIENT) && (MT_HasAdminAccess(iTank) || bHasAdminAccess(iTank)) && g_iHumanAbility[MT_GetTankType(iTank)] == 1 && (iMessage & MT_MESSAGE_RANGE) && !g_bNullify3[iTank])
	{
		g_bNullify3[iTank] = true;

		MT_PrintToChat(iTank, "%s %t", MT_TAG3, "NullifyHuman6");

		if (g_iNullifyCount[iTank] < g_iHumanAmmo[MT_GetTankType(iTank)] && g_iHumanAmmo[MT_GetTankType(iTank)] > 0)
		{
	...
	if (g_iNullifyMessage[MT_GetTankType(iTank)] & iMessage)
	{
...
public Action tTimerResetCooldown(Handle timer, int userid)
{
	...
	if (!MT_IsTankSupported(iTank, MT_CHECK_INDEX|MT_CHECK_INGAME|MT_CHECK_ALIVE|MT_CHECK_KICKQUEUE|MT_CHECK_FAKECLIENT) || !bIsCloneAllowed(iTank, g_bCloneInstalled) || !g_bNullify3[iTank])
	{
		g_bNullify3[iTank] = false;
