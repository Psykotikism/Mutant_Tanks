...
float g_flGodChance[MT_MAXTYPES + 1], g_flGodDuration[MT_MAXTYPES + 1], g_flHumanCooldown[MT_MAXTYPES + 1];

int g_iAccessFlags[MT_MAXTYPES + 1], g_iAccessFlags2[MAXPLAYERS + 1], g_iGodAbility[MT_MAXTYPES + 1], g_iGodCount[MAXPLAYERS + 1], g_iGodMessage[MT_MAXTYPES + 1], g_iHumanAbility[MT_MAXTYPES + 1], g_iHumanAmmo[MT_MAXTYPES + 1], g_iHumanMode[MT_MAXTYPES + 1], g_iImmunityFlags[MT_MAXTYPES + 1], g_iImmunityFlags2[MAXPLAYERS + 1];
...
public int iGodMenuHandler(Menu menu, MenuAction action, int param1, int param2)
{
	switch (action)
	{
		...
		case MenuAction_Select:
		{
			switch (param2)
			{
				case 0: MT_PrintToChat(param1, "%s %t", MT_TAG3, g_iGodAbility[MT_GetTankType(param1)] == 0 ? "AbilityStatus1" : "AbilityStatus2");
				case 1: MT_PrintToChat(param1, "%s %t", MT_TAG3, "AbilityAmmo", g_iHumanAmmo[MT_GetTankType(param1)] - g_iGodCount[param1], g_iHumanAmmo[MT_GetTankType(param1)]);
				...
				case 6: MT_PrintToChat(param1, "%s %t", MT_TAG3, "AbilityDuration", g_flGodDuration[MT_GetTankType(param1)]);
...
public void MT_OnConfigsLoaded(const char[] subsection, const char[] key, const char[] value, int type, int admin)
{
	...
	if (type > 0)
	{
		MT_FindAbility(type, 21, bHasAbilities(subsection, "godability", "god ability", "god_ability", "god"));
...
public void MT_OnAbilityActivated(int tank)
{
	...
	if (MT_IsTankSupported(tank) && (!MT_IsTankSupported(tank, MT_CHECK_FAKECLIENT) || g_iHumanAbility[MT_GetTankType(tank)] == 0) && bIsCloneAllowed(tank, g_bCloneInstalled) && g_iGodAbility[MT_GetTankType(tank)] == 1 && !g_bGod[tank])
	{
		vGodAbility(tank);
	}
}
...
public void MT_OnButtonPressed(int tank, int button)
{
	if (MT_IsTankSupported(tank, MT_CHECK_INDEX|MT_CHECK_INGAME|MT_CHECK_ALIVE|MT_CHECK_KICKQUEUE|MT_CHECK_FAKECLIENT) && bIsCloneAllowed(tank, g_bCloneInstalled))
	{
		...
		{
			if (g_iGodAbility[MT_GetTankType(tank)] == 1 && g_iHumanAbility[MT_GetTankType(tank)] == 1)
			{
				switch (g_iHumanMode[MT_GetTankType(tank)])
				{
					...
					case 1:
					{
						if (g_iGodCount[tank] < g_iHumanAmmo[MT_GetTankType(tank)] && g_iHumanAmmo[MT_GetTankType(tank)] > 0)
						{
							if (!g_bGod[tank] && !g_bGod2[tank])
							{
								g_bGod[tank] = true;
								...
								MT_PrintToChat(tank, "%s %t", MT_TAG3, "GodHuman", g_iGodCount[tank], g_iHumanAmmo[MT_GetTankType(tank)]);
							}
						}
						else
						{
...
public void MT_OnButtonReleased(int tank, int button)
{
	if (MT_IsTankSupported(tank, MT_CHECK_INDEX|MT_CHECK_INGAME|MT_CHECK_ALIVE|MT_CHECK_KICKQUEUE|MT_CHECK_FAKECLIENT) && bIsCloneAllowed(tank, g_bCloneInstalled))
	{
		if (button & MT_MAIN_KEY == MT_MAIN_KEY)
		{
			if (g_iGodAbility[MT_GetTankType(tank)] == 1 && g_iHumanAbility[MT_GetTankType(tank)] == 1)
			{
				if (g_iHumanMode[MT_GetTankType(tank)] == 1 && g_bGod[tank] && !g_bGod2[tank])
				{
					g_bGod[tank] = false;
...
static void vGodAbility(int tank)
{
	...
	if (g_iGodCount[tank] < g_iHumanAmmo[MT_GetTankType(tank)] && g_iHumanAmmo[MT_GetTankType(tank)] > 0)
	{
		if (GetRandomFloat(0.1, 100.0) <= g_flGodChance[MT_GetTankType(tank)])
		{
			g_bGod[tank] = true;

			CreateTimer(g_flGodDuration[MT_GetTankType(tank)], tTimerStopGod, GetClientUserId(tank), TIMER_FLAG_NO_MAPCHANGE);
			...
			if (g_iGodMessage[MT_GetTankType(tank)] == 1)
			{
				char sTankName[33];
...
public Action tTimerStopGod(Handle timer, int userid)
{
	...
	if (MT_IsTankSupported(iTank, MT_CHECK_FAKECLIENT) && (MT_HasAdminAccess(iTank) || bHasAdminAccess(iTank)) && g_iHumanAbility[MT_GetTankType(iTank)] == 1 && !g_bGod2[iTank])
	{
		vReset2(iTank);
	}

	if (g_iGodMessage[MT_GetTankType(iTank)] == 1)
	{
		char sTankName[33];
...
{
	int iTank = GetClientOfUserId(userid);
	if (!MT_IsTankSupported(iTank, MT_CHECK_INDEX|MT_CHECK_INGAME|MT_CHECK_ALIVE|MT_CHECK_KICKQUEUE|MT_CHECK_FAKECLIENT) || !bIsCloneAllowed(iTank, g_bCloneInstalled) || !g_bGod2[iTank])
	{
		g_bGod2[iTank] = false;
