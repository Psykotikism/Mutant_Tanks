...
#define MT_CONFIG_SECTION "godability"
#define MT_CONFIG_SECTION2 "god ability"
#define MT_CONFIG_SECTION3 "god_ability"
#define MT_CONFIG_SECTION4 "god"
...
public Action OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype)
{
	...
	{
		if (MT_IsTankSupported(victim) && MT_IsCustomTankSupported(victim) && bIsSurvivor(attacker))
		{
...
public void MT_OnCombineAbilities(int tank, int type, float random, const char[] combo, int survivor, int weapon, const char[] classname)
{
	...
	{
		if (type == MT_COMBO_MAINRANGE && g_esCache[tank].g_iGodAbility == 1 && g_esCache[tank].g_iComboAbility == 1 && !g_esPlayer[tank].g_bActivated)
		{
...
public void MT_OnConfigsLoaded(const char[] subsection, const char[] key, const char[] value, int type, int admin, int mode)
{
	if (mode == 3 && bIsValidClient(admin))
	{
		...
		g_esPlayer[admin].g_iGodAbility = iGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "AbilityEnabled", "Ability Enabled", "Ability_Enabled", "aenabled", g_esPlayer[admin].g_iGodAbility, value, 0, 1);
		g_esPlayer[admin].g_iGodMessage = iGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "AbilityMessage", "Ability Message", "Ability_Message", "message", g_esPlayer[admin].g_iGodMessage, value, 0, 1);
		g_esPlayer[admin].g_flGodChance = flGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "GodChance", "God Chance", "God_Chance", "chance", g_esPlayer[admin].g_flGodChance, value, 0.0, 100.0);
		g_esPlayer[admin].g_iGodDuration = iGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "GodDuration", "God Duration", "God_Duration", "duration", g_esPlayer[admin].g_iGodDuration, value, 1, 999999);
	...
	if (mode < 3 && type > 0)
	{
		...
		g_esAbility[type].g_iGodAbility = iGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "AbilityEnabled", "Ability Enabled", "Ability_Enabled", "aenabled", g_esAbility[type].g_iGodAbility, value, 0, 1);
		g_esAbility[type].g_iGodMessage = iGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "AbilityMessage", "Ability Message", "Ability_Message", "message", g_esAbility[type].g_iGodMessage, value, 0, 1);
		g_esAbility[type].g_flGodChance = flGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "GodChance", "God Chance", "God_Chance", "chance", g_esAbility[type].g_flGodChance, value, 0.0, 100.0);
		g_esAbility[type].g_iGodDuration = iGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "GodDuration", "God Duration", "God_Duration", "duration", g_esAbility[type].g_iGodDuration, value, 1, 999999);
...
public void MT_OnAbilityActivated(int tank)
{
	...
	if (MT_IsTankSupported(tank) && (!MT_IsTankSupported(tank, MT_CHECK_FAKECLIENT) || g_esCache[tank].g_iHumanAbility != 1) && MT_IsCustomTankSupported(tank) && g_esCache[tank].g_iGodAbility == 1 && g_esCache[tank].g_iComboAbility == 0 && !g_esPlayer[tank].g_bActivated)
	{
		vGodAbility(tank);
	}
}
					...
					case 1:
					{
						if (g_esPlayer[tank].g_iAmmoCount < g_esCache[tank].g_iHumanAmmo && g_esCache[tank].g_iHumanAmmo > 0)
						{
							if (!g_esPlayer[tank].g_bActivated && !bRecharging)
							{
								...
								MT_PrintToChat(tank, "%s %t", MT_TAG3, "GodHuman", g_esPlayer[tank].g_iAmmoCount, g_esCache[tank].g_iHumanAmmo);
							}
							else if (g_esPlayer[tank].g_bActivated)
							{
								MT_PrintToChat(tank, "%s %t", MT_TAG3, "GodHuman3");
...
static void vGod(int tank, int pos = -1)
{
	static int iDuration;
	iDuration = (pos != -1) ? RoundToNearest(MT_GetCombinationSetting(tank, 4, pos)) : g_esCache[tank].g_iGodDuration;
	g_esPlayer[tank].g_bActivated = true;
...
public Action tTimerCombo(Handle timer, DataPack pack)
{
	...
	if (!MT_IsCorePluginEnabled() || !MT_IsTankSupported(iTank) || (!MT_HasAdminAccess(iTank) && !bHasAdminAccess(iTank, g_esAbility[g_esPlayer[iTank].g_iTankType].g_iAccessFlags, g_esPlayer[iTank].g_iAccessFlags)) || !MT_IsTypeEnabled(g_esPlayer[iTank].g_iTankType) || !MT_IsCustomTankSupported(iTank) || g_esCache[iTank].g_iGodAbility == 0 || g_esPlayer[iTank].g_bActivated)
	{
		return Plugin_Stop;
	}
