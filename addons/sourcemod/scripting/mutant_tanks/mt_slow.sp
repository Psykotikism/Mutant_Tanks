...
public Action OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype)
{
	if (MT_IsCorePluginEnabled() && bIsValidClient(victim, MT_CHECK_INDEX|MT_CHECK_INGAME|MT_CHECK_ALIVE|MT_CHECK_INKICKQUEUE) && damage >= 0.5)
	{
		...
		if (MT_IsTankSupported(attacker) && MT_IsCustomTankSupported(attacker) && (g_esCache[attacker].g_iSlowHitMode == 0 || g_esCache[attacker].g_iSlowHitMode == 1) && bIsSurvivor(victim))
		{
		...
		else if (MT_IsTankSupported(victim) && MT_IsCustomTankSupported(victim) && (g_esCache[victim].g_iSlowHitMode == 0 || g_esCache[victim].g_iSlowHitMode == 2) && bIsSurvivor(attacker))
		{
...
public void MT_OnConfigsLoaded(const char[] subsection, const char[] key, const char[] value, int type, int admin, int mode)
{
	if (mode == 3 && bIsValidClient(admin))
	{
		...
		g_esPlayer[admin].g_iOpenAreasOnly = iGetKeyValue(subsection, "slowability", "slow ability", "slow_ability", "slow", key, "OpenAreasOnly", "Open Areas Only", "Open_Areas_Only", "openareas", g_esPlayer[admin].g_iOpenAreasOnly, value, 0, 1);
		...
		g_esPlayer[admin].g_iSlowMessage = iGetKeyValue(subsection, "slowability", "slow ability", "slow_ability", "slow", key, "AbilityMessage", "Ability Message", "Ability_Message", "message", g_esPlayer[admin].g_iSlowMessage, value, 0, 3);
	...
	if (mode < 3 && type > 0)
	{
		...
		g_esAbility[type].g_iOpenAreasOnly = iGetKeyValue(subsection, "slowability", "slow ability", "slow_ability", "slow", key, "OpenAreasOnly", "Open Areas Only", "Open_Areas_Only", "openareas", g_esAbility[type].g_iOpenAreasOnly, value, 0, 1);
		...
		g_esAbility[type].g_iSlowMessage = iGetKeyValue(subsection, "slowability", "slow ability", "slow_ability", "slow", key, "AbilityMessage", "Ability Message", "Ability_Message", "message", g_esAbility[type].g_iSlowMessage, value, 0, 3);
...
public void MT_OnAbilityActivated(int tank)
{
	...
	if (MT_IsTankSupported(tank) && (!MT_IsTankSupported(tank, MT_CHECK_FAKECLIENT) || g_esCache[tank].g_iHumanAbility != 1) && MT_IsCustomTankSupported(tank) && g_esCache[tank].g_iSlowAbility == 1)
	{
		vSlowAbility(tank);
	}
}
		...
		if (!MT_IsTankSupported(tank, MT_CHECK_FAKECLIENT) || (g_esPlayer[tank].g_iCount < g_esCache[tank].g_iHumanAmmo && g_esCache[tank].g_iHumanAmmo > 0))
		{
			...
			if (GetRandomFloat(0.1, 100.0) <= chance && !g_esPlayer[survivor].g_bAffected)
			{
				...
				if (g_esCache[tank].g_iSlowMessage & messages)
				{
					...
					MT_LogMessage(MT_LOG_ABILITY, "%s %T", MT_TAG, "Slow", LANG_SERVER, sTankName, survivor, g_esCache[tank].g_flSlowSpeed);
				}
			}
