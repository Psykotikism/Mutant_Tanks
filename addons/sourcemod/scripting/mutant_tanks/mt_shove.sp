...
#define MT_CONFIG_SECTION "shoveability"
#define MT_CONFIG_SECTION2 "shove ability"
#define MT_CONFIG_SECTION3 "shove_ability"
#define MT_CONFIG_SECTION4 "shove"
...
public Action OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype)
{
	if (MT_IsCorePluginEnabled() && bIsValidClient(victim, MT_CHECK_INDEX|MT_CHECK_INGAME|MT_CHECK_ALIVE) && bIsValidEntity(inflictor) && damage > 0.0)
	{
		...
		if (MT_IsTankSupported(attacker) && MT_IsCustomTankSupported(attacker) && (g_esCache[attacker].g_iShoveHitMode == 0 || g_esCache[attacker].g_iShoveHitMode == 1) && bIsSurvivor(victim) && g_esCache[attacker].g_iComboAbility == 0)
		{
			...
			{
				vShoveHit(victim, attacker, GetRandomFloat(0.1, 100.0), g_esCache[attacker].g_flShoveChance, g_esCache[attacker].g_iShoveHit, MT_MESSAGE_MELEE, MT_ATTACK_CLAW);
			}
		}
		else if (MT_IsTankSupported(victim) && MT_IsCustomTankSupported(victim) && (g_esCache[victim].g_iShoveHitMode == 0 || g_esCache[victim].g_iShoveHitMode == 2) && bIsSurvivor(attacker) && g_esCache[victim].g_iComboAbility == 0)
		{
			...
			{
				vShoveHit(attacker, victim, GetRandomFloat(0.1, 100.0), g_esCache[victim].g_flShoveChance, g_esCache[victim].g_iShoveHit, MT_MESSAGE_MELEE, MT_ATTACK_MELEE);
			}
		}
	}
		...
		for (int iPos = 0; iPos < sizeof(sSubset); iPos++)
		{
			if (StrEqual(sSubset[iPos], MT_CONFIG_SECTION, false) || StrEqual(sSubset[iPos], MT_CONFIG_SECTION2, false) || StrEqual(sSubset[iPos], MT_CONFIG_SECTION3, false) || StrEqual(sSubset[iPos], MT_CONFIG_SECTION4, false))
			{
				...
				switch (type)
				{
					...
					{
						if (g_esCache[tank].g_iShoveAbility == 1)
						{
							switch (flDelay)
							{
					...
					case MT_COMBO_MELEEHIT:
					{
						...
						switch (flDelay)
						{
							case 0.0:
							{
								if ((g_esCache[tank].g_iShoveHitMode == 0 || g_esCache[tank].g_iShoveHitMode == 1) && (StrEqual(classname, "weapon_tank_claw") || StrEqual(classname, "tank_rock")))
								{
									vShoveHit(survivor, tank, random, flChance, g_esCache[tank].g_iShoveHit, MT_MESSAGE_MELEE, MT_ATTACK_CLAW, iPos);
								}
								else if ((g_esCache[tank].g_iShoveHitMode == 0 || g_esCache[tank].g_iShoveHitMode == 2) && StrEqual(classname, "weapon_melee"))
								{
									vShoveHit(survivor, tank, random, flChance, g_esCache[tank].g_iShoveHit, MT_MESSAGE_MELEE, MT_ATTACK_MELEE, iPos);
								}
							}
							default:
							{
...
public void MT_OnConfigsLoaded(const char[] subsection, const char[] key, const char[] value, int type, int admin, int mode)
{
	if (mode == 3 && bIsValidClient(admin))
	{
		...
		g_esPlayer[admin].g_iShoveAbility = iGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "AbilityEnabled", "Ability Enabled", "Ability_Enabled", "aenabled", g_esPlayer[admin].g_iShoveAbility, value, 0, 1);
		g_esPlayer[admin].g_iShoveEffect = iGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "AbilityEffect", "Ability Effect", "Ability_Effect", "effect", g_esPlayer[admin].g_iShoveEffect, value, 0, 7);
		g_esPlayer[admin].g_iShoveMessage = iGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "AbilityMessage", "Ability Message", "Ability_Message", "message", g_esPlayer[admin].g_iShoveMessage, value, 0, 3);
		g_esPlayer[admin].g_flShoveChance = flGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "ShoveChance", "Shove Chance", "Shove_Chance", "chance", g_esPlayer[admin].g_flShoveChance, value, 0.0, 100.0);
		g_esPlayer[admin].g_iShoveDeath = iGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "ShoveDeath", "Shove Death", "Shove_Death", "death", g_esPlayer[admin].g_iShoveDeath, value, 0, 1);
		g_esPlayer[admin].g_flShoveDeathChance = flGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "ShoveDeathChance", "Shove Death Chance", "Shove_Death_Chance", "deathchance", g_esPlayer[admin].g_flShoveDeathChance, value, 0.0, 100.0);
		g_esPlayer[admin].g_flShoveDeathRange = flGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "ShoveDeathRange", "Shove Death Range", "Shove_Death_Range", "deathrange", g_esPlayer[admin].g_flShoveDeathRange, value, 1.0, 999999.0);
		g_esPlayer[admin].g_iShoveDuration = iGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "ShoveDuration", "Shove Duration", "Shove_Duration", "duration", g_esPlayer[admin].g_iShoveDuration, value, 1, 999999);
		g_esPlayer[admin].g_iShoveHit = iGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "ShoveHit", "Shove Hit", "Shove_Hit", "hit", g_esPlayer[admin].g_iShoveHit, value, 0, 1);
		g_esPlayer[admin].g_iShoveHitMode = iGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "ShoveHitMode", "Shove Hit Mode", "Shove_Hit_Mode", "hitmode", g_esPlayer[admin].g_iShoveHitMode, value, 0, 2);
		g_esPlayer[admin].g_flShoveInterval = flGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "ShoveInterval", "Shove Interval", "Shove_Interval", "interval", g_esPlayer[admin].g_flShoveInterval, value, 0.1, 999999.0);
		g_esPlayer[admin].g_flShoveRange = flGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "ShoveRange", "Shove Range", "Shove_Range", "range", g_esPlayer[admin].g_flShoveRange, value, 1.0, 999999.0);
		g_esPlayer[admin].g_flShoveRangeChance = flGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "ShoveRangeChance", "Shove Range Chance", "Shove_Range_Chance", "rangechance", g_esPlayer[admin].g_flShoveRangeChance, value, 0.0, 100.0);
	...
	if (mode < 3 && type > 0)
	{
		...
		g_esAbility[type].g_iShoveAbility = iGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "AbilityEnabled", "Ability Enabled", "Ability_Enabled", "aenabled", g_esAbility[type].g_iShoveAbility, value, 0, 1);
		g_esAbility[type].g_iShoveEffect = iGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "AbilityEffect", "Ability Effect", "Ability_Effect", "effect", g_esAbility[type].g_iShoveEffect, value, 0, 7);
		g_esAbility[type].g_iShoveMessage = iGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "AbilityMessage", "Ability Message", "Ability_Message", "message", g_esAbility[type].g_iShoveMessage, value, 0, 3);
		g_esAbility[type].g_flShoveChance = flGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "ShoveChance", "Shove Chance", "Shove_Chance", "chance", g_esAbility[type].g_flShoveChance, value, 0.0, 100.0);
		g_esAbility[type].g_iShoveDeath = iGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "ShoveDeath", "Shove Death", "Shove_Death", "death", g_esAbility[type].g_iShoveDeath, value, 0, 1);
		g_esAbility[type].g_flShoveDeathChance = flGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "ShoveDeathChance", "Shove Death Chance", "Shove_Death_Chance", "deathchance", g_esAbility[type].g_flShoveDeathChance, value, 0.0, 100.0);
		g_esAbility[type].g_flShoveDeathRange = flGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "ShoveDeathRange", "Shove Death Range", "Shove_Death_Range", "deathrange", g_esAbility[type].g_flShoveDeathRange, value, 1.0, 999999.0);
		g_esAbility[type].g_iShoveDuration = iGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "ShoveDuration", "Shove Duration", "Shove_Duration", "duration", g_esAbility[type].g_iShoveDuration, value, 1, 999999);
		g_esAbility[type].g_iShoveHit = iGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "ShoveHit", "Shove Hit", "Shove_Hit", "hit", g_esAbility[type].g_iShoveHit, value, 0, 1);
		g_esAbility[type].g_iShoveHitMode = iGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "ShoveHitMode", "Shove Hit Mode", "Shove_Hit_Mode", "hitmode", g_esAbility[type].g_iShoveHitMode, value, 0, 2);
		g_esAbility[type].g_flShoveInterval = flGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "ShoveInterval", "Shove Interval", "Shove_Interval", "interval", g_esAbility[type].g_flShoveInterval, value, 0.1, 999999.0);
		g_esAbility[type].g_flShoveRange = flGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "ShoveRange", "Shove Range", "Shove_Range", "range", g_esAbility[type].g_flShoveRange, value, 1.0, 999999.0);
		g_esAbility[type].g_flShoveRangeChance = flGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "ShoveRangeChance", "Shove Range Chance", "Shove_Range_Chance", "rangechance", g_esAbility[type].g_flShoveRangeChance, value, 0.0, 100.0);
...
public void MT_OnAbilityActivated(int tank)
{
	...
	if (MT_IsTankSupported(tank) && (!bIsTank(tank, MT_CHECK_FAKECLIENT) || g_esCache[tank].g_iHumanAbility != 1) && MT_IsCustomTankSupported(tank) && g_esCache[tank].g_iShoveAbility == 1 && g_esCache[tank].g_iComboAbility == 0)
	{
...
static void vShoveAbility(int tank, float random, int pos = -1)
{
	if (bIsAreaNarrow(tank, g_esCache[tank].g_flOpenAreasOnly) || MT_DoesTypeRequireHumans(g_esPlayer[tank].g_iTankType) || (g_esCache[tank].g_iRequiresHumans > 0 && iGetHumanCount() < g_esCache[tank].g_iRequiresHumans) || (!MT_HasAdminAccess(tank) && !bHasAdminAccess(tank, g_esAbility[g_esPlayer[tank].g_iTankType].g_iAccessFlags, g_esPlayer[tank].g_iAccessFlags)))
	{
		return;
	...
	if (!bIsTank(tank, MT_CHECK_FAKECLIENT) || (g_esPlayer[tank].g_iAmmoCount < g_esCache[tank].g_iHumanAmmo && g_esCache[tank].g_iHumanAmmo > 0))
	{
		...
		flRange = (pos != -1) ? MT_GetCombinationSetting(tank, 8, pos) : g_esCache[tank].g_flShoveRange;
		flChance = (pos != -1) ? MT_GetCombinationSetting(tank, 9, pos) : g_esCache[tank].g_flShoveRangeChance;
		...
		for (int iSurvivor = 1; iSurvivor <= MaxClients; iSurvivor++)
		{
			if (bIsSurvivor(iSurvivor, MT_CHECK_INGAME|MT_CHECK_ALIVE) && !MT_IsAdminImmune(iSurvivor, tank) && !bIsAdminImmune(iSurvivor, g_esPlayer[tank].g_iTankType, g_esAbility[g_esPlayer[tank].g_iTankType].g_iImmunityFlags, g_esPlayer[iSurvivor].g_iImmunityFlags))
			{
				...
				{
					vShoveHit(iSurvivor, tank, random, flChance, g_esCache[tank].g_iShoveAbility, MT_MESSAGE_RANGE, MT_ATTACK_RANGE, pos);
...
static void vShoveHit(int survivor, int tank, float random, float chance, int enabled, int messages, int flags, int pos = -1)
{
	...
	if (enabled == 1 && bIsSurvivor(survivor) && !bIsPlayerDisabled(survivor) && !g_esPlayer[survivor].g_bRewarded && g_hSDKStagger != null)
	{
		if (!bIsTank(tank, MT_CHECK_FAKECLIENT) || (g_esPlayer[tank].g_iAmmoCount < g_esCache[tank].g_iHumanAmmo && g_esCache[tank].g_iHumanAmmo > 0))
		{
			static int iTime;
			...
			if (random <= chance && !g_esPlayer[survivor].g_bAffected)
			{
				...
				if (bIsTank(tank, MT_CHECK_FAKECLIENT) && g_esCache[tank].g_iHumanAbility == 1 && (flags & MT_ATTACK_RANGE) && (g_esPlayer[tank].g_iCooldown == -1 || g_esPlayer[tank].g_iCooldown < iTime))
				{
					...
					MT_PrintToChat(tank, "%s %t", MT_TAG3, "ShoveHuman", g_esPlayer[tank].g_iAmmoCount, g_esCache[tank].g_iHumanAmmo);
				...
				flInterval = (pos != -1) ? MT_GetCombinationSetting(tank, 5, pos) : g_esCache[tank].g_flShoveInterval;
				DataPack dpShove;
...
public Action tTimerCombo(Handle timer, DataPack pack)
{
	...
	if (!MT_IsCorePluginEnabled() || !MT_IsTankSupported(iTank) || (!MT_HasAdminAccess(iTank) && !bHasAdminAccess(iTank, g_esAbility[g_esPlayer[iTank].g_iTankType].g_iAccessFlags, g_esPlayer[iTank].g_iAccessFlags)) || !MT_IsTypeEnabled(g_esPlayer[iTank].g_iTankType) || !MT_IsCustomTankSupported(iTank) || g_esCache[iTank].g_iShoveAbility == 0)
	{
		return Plugin_Stop;
	}
...
public Action tTimerCombo2(Handle timer, DataPack pack)
{
	...
	int iTank = GetClientOfUserId(pack.ReadCell());
	if (!MT_IsCorePluginEnabled() || !MT_IsTankSupported(iTank) || (!MT_HasAdminAccess(iTank) && !bHasAdminAccess(iTank, g_esAbility[g_esPlayer[iTank].g_iTankType].g_iAccessFlags, g_esPlayer[iTank].g_iAccessFlags)) || !MT_IsTypeEnabled(g_esPlayer[iTank].g_iTankType) || !MT_IsCustomTankSupported(iTank) || g_esCache[iTank].g_iShoveHit == 0)
	{
		return Plugin_Stop;
	}
	...
	if ((g_esCache[iTank].g_iShoveHitMode == 0 || g_esCache[iTank].g_iShoveHitMode == 1) && (StrEqual(sClassname, "weapon_tank_claw") || StrEqual(sClassname, "tank_rock")))
	{
		vShoveHit(iSurvivor, iTank, flRandom, flChance, g_esCache[iTank].g_iShoveHit, MT_MESSAGE_MELEE, MT_ATTACK_CLAW, iPos);
	}
	else if ((g_esCache[iTank].g_iShoveHitMode == 0 || g_esCache[iTank].g_iShoveHitMode == 2) && StrEqual(sClassname, "weapon_melee"))
	{
		vShoveHit(iSurvivor, iTank, flRandom, flChance, g_esCache[iTank].g_iShoveHit, MT_MESSAGE_MELEE, MT_ATTACK_MELEE, iPos);
	}
...
public Action tTimerShove(Handle timer, DataPack pack)
{
	...
	if (!MT_IsTankSupported(iTank) || bIsAreaNarrow(iTank, g_esCache[iTank].g_flOpenAreasOnly) || MT_DoesTypeRequireHumans(g_esPlayer[iTank].g_iTankType) || (g_esCache[iTank].g_iRequiresHumans > 0 && iGetHumanCount() < g_esCache[iTank].g_iRequiresHumans) || (!MT_HasAdminAccess(iTank) && !bHasAdminAccess(iTank, g_esAbility[g_esPlayer[iTank].g_iTankType].g_iAccessFlags, g_esPlayer[iTank].g_iAccessFlags)) || !MT_IsTypeEnabled(g_esPlayer[iTank].g_iTankType) || !MT_IsCustomTankSupported(iTank) || iType != g_esPlayer[iTank].g_iTankType || MT_IsAdminImmune(iSurvivor, iTank) || bIsAdminImmune(iSurvivor, g_esPlayer[iTank].g_iTankType, g_esAbility[g_esPlayer[iTank].g_iTankType].g_iImmunityFlags, g_esPlayer[iSurvivor].g_iImmunityFlags) || !g_esPlayer[iSurvivor].g_bAffected || g_esPlayer[iSurvivor].g_bRewarded)
	{
	...
	iPos = pack.ReadCell();
	iDuration = (iPos != -1) ? RoundToNearest(MT_GetCombinationSetting(iTank, 4, iPos)) : g_esCache[iTank].g_iShoveDuration;
	iTime = pack.ReadCell();
