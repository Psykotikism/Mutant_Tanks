...
#define MT_CONFIG_SECTION "laserability"
#define MT_CONFIG_SECTION2 "laser ability"
#define MT_CONFIG_SECTION3 "laser_ability"
#define MT_CONFIG_SECTION4 "laser"
...
public void MT_OnCombineAbilities(int tank, int type, float random, const char[] combo, int survivor, int weapon, const char[] classname)
{
	...
	{
		if (type == MT_COMBO_MAINRANGE && g_esCache[tank].g_iLaserAbility == 1 && g_esCache[tank].g_iComboAbility == 1 && !g_esPlayer[tank].g_bActivated)
		{
...
public void MT_OnConfigsLoaded(const char[] subsection, const char[] key, const char[] value, int type, int admin, int mode)
{
	if (mode == 3 && bIsValidClient(admin))
	{
		...
		g_esPlayer[admin].g_iLaserAbility = iGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "AbilityEnabled", "Ability Enabled", "Ability_Enabled", "aenabled", g_esPlayer[admin].g_iLaserAbility, value, 0, 1);
		g_esPlayer[admin].g_iLaserMessage = iGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "AbilityMessage", "Ability Message", "Ability_Message", "message", g_esPlayer[admin].g_iLaserMessage, value, 0, 1);
		g_esPlayer[admin].g_flLaserChance = flGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "LaserChance", "Laser Chance", "Laser_Chance", "chance", g_esPlayer[admin].g_flLaserChance, value, 0.0, 100.0);
		g_esPlayer[admin].g_flLaserDamage = flGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "LaserDamage", "Laser Damage", "Laser_Damage", "damage", g_esPlayer[admin].g_flLaserDamage, value, 0.1, 999999.0);
		g_esPlayer[admin].g_iLaserDuration = iGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "LaserDuration", "Laser Duration", "Laser_Duration", "duration", g_esPlayer[admin].g_iLaserDuration, value, 1, 999999);
		g_esPlayer[admin].g_flLaserInterval = flGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "LaserInterval", "Laser Interval", "Laser_Interval", "interval", g_esPlayer[admin].g_flLaserInterval, value, 0.1, 999999.0);
		g_esPlayer[admin].g_flLaserRange = flGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "LaserRange", "Laser Range", "Laser_Range", "range", g_esPlayer[admin].g_flLaserRange, value, 0.1, 999999.0);
	...
	if (mode < 3 && type > 0)
	{
		...
		g_esAbility[type].g_iLaserAbility = iGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "AbilityEnabled", "Ability Enabled", "Ability_Enabled", "aenabled", g_esAbility[type].g_iLaserAbility, value, 0, 1);
		g_esAbility[type].g_iLaserMessage = iGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "AbilityMessage", "Ability Message", "Ability_Message", "message", g_esAbility[type].g_iLaserMessage, value, 0, 1);
		g_esAbility[type].g_flLaserChance = flGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "LaserChance", "Laser Chance", "Laser_Chance", "chance", g_esAbility[type].g_flLaserChance, value, 0.0, 100.0);
		g_esAbility[type].g_flLaserDamage = flGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "LaserDamage", "Laser Damage", "Laser_Damage", "damage", g_esAbility[type].g_flLaserDamage, value, 0.1, 999999.0);
		g_esAbility[type].g_iLaserDuration = iGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "LaserDuration", "Laser Duration", "Laser_Duration", "duration", g_esAbility[type].g_iLaserDuration, value, 1, 999999);
		g_esAbility[type].g_flLaserInterval = flGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "LaserInterval", "Laser Interval", "Laser_Interval", "interval", g_esAbility[type].g_flLaserInterval, value, 0.1, 999999.0);
		g_esAbility[type].g_flLaserRange = flGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "LaserRange", "Laser Range", "Laser_Range", "range", g_esAbility[type].g_flLaserRange, value, 0.1, 999999.0);
...
public void MT_OnAbilityActivated(int tank)
{
	...
	if (MT_IsTankSupported(tank) && (!MT_IsTankSupported(tank, MT_CHECK_FAKECLIENT) || g_esCache[tank].g_iHumanAbility != 1) && MT_IsCustomTankSupported(tank) && g_esCache[tank].g_iLaserAbility == 1 && g_esCache[tank].g_iComboAbility == 0 && !g_esPlayer[tank].g_bActivated)
	{
		vLaserAbility(tank);
	}
}
					...
					case 1:
					{
						if (g_esPlayer[tank].g_iAmmoCount < g_esCache[tank].g_iHumanAmmo && g_esCache[tank].g_iHumanAmmo > 0)
						{
							if (!g_esPlayer[tank].g_bActivated && !bRecharging)
							{
								...
								MT_PrintToChat(tank, "%s %t", MT_TAG3, "LaserHuman", g_esPlayer[tank].g_iAmmoCount, g_esCache[tank].g_iHumanAmmo);
							}
							else if (g_esPlayer[tank].g_bActivated)
							{
								MT_PrintToChat(tank, "%s %t", MT_TAG3, "LaserHuman3");
...
static void vLaser(int tank, int pos = -1)
{
	g_esPlayer[tank].g_bActivated = true;
	...
	flInterval = (pos != -1) ? MT_GetCombinationSetting(tank, 5, pos) : g_esCache[tank].g_flLaserInterval;
	DataPack dpLaser;
...
static void vLaser2(int tank, int pos = -1)
{
...
public Action tTimerCombo(Handle timer, DataPack pack)
{
	...
	if (!MT_IsCorePluginEnabled() || !MT_IsTankSupported(iTank) || (!MT_HasAdminAccess(iTank) && !bHasAdminAccess(iTank, g_esAbility[g_esPlayer[iTank].g_iTankType].g_iAccessFlags, g_esPlayer[iTank].g_iAccessFlags)) || !MT_IsTypeEnabled(g_esPlayer[iTank].g_iTankType) || !MT_IsCustomTankSupported(iTank) || g_esCache[iTank].g_iLaserAbility == 0 || g_esPlayer[iTank].g_bActivated)
	{
		return Plugin_Stop;
	}
...
public Action tTimerLaser(Handle timer, DataPack pack)
{
	...
	iTime = pack.ReadCell();
	iPos = pack.ReadCell();
	iDuration = (iPos != -1) ? RoundToNearest(MT_GetCombinationSetting(iTank, 4, iPos)) : g_esCache[iTank].g_iLaserDuration;
