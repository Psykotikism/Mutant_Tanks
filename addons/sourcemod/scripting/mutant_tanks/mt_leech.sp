...
#define MT_CONFIG_SECTION "leechability"
#define MT_CONFIG_SECTION2 "leech ability"
#define MT_CONFIG_SECTION3 "leech_ability"
#define MT_CONFIG_SECTION4 "leech"
...
public Action OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype)
{
	if (MT_IsCorePluginEnabled() && bIsValidClient(victim, MT_CHECK_INDEX|MT_CHECK_INGAME|MT_CHECK_ALIVE) && bIsValidEntity(inflictor) && damage > 0.0)
	{
		...
		if (MT_IsTankSupported(attacker) && MT_IsCustomTankSupported(attacker) && (g_esCache[attacker].g_iLeechHitMode == 0 || g_esCache[attacker].g_iLeechHitMode == 1) && bIsSurvivor(victim) && g_esCache[attacker].g_iComboAbility == 0)
		{
			...
			{
				vLeechHit(victim, attacker, GetRandomFloat(0.1, 100.0), g_esCache[attacker].g_flLeechChance, g_esCache[attacker].g_iLeechHit, MT_MESSAGE_MELEE, MT_ATTACK_CLAW);
			}
		}
		else if (MT_IsTankSupported(victim) && MT_IsCustomTankSupported(victim) && (g_esCache[victim].g_iLeechHitMode == 0 || g_esCache[victim].g_iLeechHitMode == 2) && bIsSurvivor(attacker) && g_esCache[victim].g_iComboAbility == 0)
		{
			...
			{
				vLeechHit(attacker, victim, GetRandomFloat(0.1, 100.0), g_esCache[victim].g_flLeechChance, g_esCache[victim].g_iLeechHit, MT_MESSAGE_MELEE, MT_ATTACK_MELEE);
			}
		}
	}
		...
		for (int iPos = 0; iPos < sizeof(sSubset); iPos++)
		{
			if (StrEqual(sSubset[iPos], MT_CONFIG_SECTION, false) || StrEqual(sSubset[iPos], MT_CONFIG_SECTION2, false) || StrEqual(sSubset[iPos], MT_CONFIG_SECTION3, false) || StrEqual(sSubset[iPos], MT_CONFIG_SECTION4, false))
			{
				...
				switch (type)
				{
					...
					{
						if (g_esCache[tank].g_iLeechAbility == 1)
						{
							switch (flDelay)
							{
					...
					case MT_COMBO_MELEEHIT:
					{
						...
						switch (flDelay)
						{
							case 0.0:
							{
								if ((g_esCache[tank].g_iLeechHitMode == 0 || g_esCache[tank].g_iLeechHitMode == 1) && (StrEqual(classname, "weapon_tank_claw") || StrEqual(classname, "tank_rock")))
								{
									vLeechHit(survivor, tank, random, flChance, g_esCache[tank].g_iLeechHit, MT_MESSAGE_MELEE, MT_ATTACK_CLAW, iPos);
								}
								else if ((g_esCache[tank].g_iLeechHitMode == 0 || g_esCache[tank].g_iLeechHitMode == 2) && StrEqual(classname, "weapon_melee"))
								{
									vLeechHit(survivor, tank, random, flChance, g_esCache[tank].g_iLeechHit, MT_MESSAGE_MELEE, MT_ATTACK_MELEE, iPos);
								}
							}
							default:
							{
...
public void MT_OnConfigsLoaded(const char[] subsection, const char[] key, const char[] value, int type, int admin, int mode)
{
	if (mode == 3 && bIsValidClient(admin))
	{
		...
		g_esPlayer[admin].g_iLeechAbility = iGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "AbilityEnabled", "Ability Enabled", "Ability_Enabled", "aenabled", g_esPlayer[admin].g_iLeechAbility, value, 0, 1);
		g_esPlayer[admin].g_iLeechEffect = iGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "AbilityEffect", "Ability Effect", "Ability_Effect", "effect", g_esPlayer[admin].g_iLeechEffect, value, 0, 7);
		g_esPlayer[admin].g_iLeechMessage = iGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "AbilityMessage", "Ability Message", "Ability_Message", "message", g_esPlayer[admin].g_iLeechMessage, value, 0, 3);
		g_esPlayer[admin].g_flLeechChance = flGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "LeechChance", "Leech Chance", "Leech_Chance", "chance", g_esPlayer[admin].g_flLeechChance, value, 0.0, 100.0);
		g_esPlayer[admin].g_iLeechDuration = iGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "LeechDuration", "Leech Duration", "Leech_Duration", "duration", g_esPlayer[admin].g_iLeechDuration, value, 1, 999999);
		g_esPlayer[admin].g_iLeechHit = iGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "LeechHit", "Leech Hit", "Leech_Hit", "hit", g_esPlayer[admin].g_iLeechHit, value, 0, 1);
		g_esPlayer[admin].g_iLeechHitMode = iGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "LeechHitMode", "Leech Hit Mode", "Leech_Hit_Mode", "hitmode", g_esPlayer[admin].g_iLeechHitMode, value, 0, 2);
		g_esPlayer[admin].g_flLeechInterval = flGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "LeechInterval", "Leech Interval", "Leech_Interval", "interval", g_esPlayer[admin].g_flLeechInterval, value, 0.1, 999999.0);
		g_esPlayer[admin].g_flLeechRange = flGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "LeechRange", "Leech Range", "Leech_Range", "range", g_esPlayer[admin].g_flLeechRange, value, 1.0, 999999.0);
		g_esPlayer[admin].g_flLeechRangeChance = flGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "LeechRangeChance", "Leech Range Chance", "Leech_Range_Chance", "rangechance", g_esPlayer[admin].g_flLeechRangeChance, value, 0.0, 100.0);
	...
	if (mode < 3 && type > 0)
	{
		...
		g_esAbility[type].g_iLeechAbility = iGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "AbilityEnabled", "Ability Enabled", "Ability_Enabled", "aenabled", g_esAbility[type].g_iLeechAbility, value, 0, 1);
		g_esAbility[type].g_iLeechEffect = iGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "AbilityEffect", "Ability Effect", "Ability_Effect", "effect", g_esAbility[type].g_iLeechEffect, value, 0, 7);
		g_esAbility[type].g_iLeechMessage = iGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "AbilityMessage", "Ability Message", "Ability_Message", "message", g_esAbility[type].g_iLeechMessage, value, 0, 3);
		g_esAbility[type].g_flLeechChance = flGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "LeechChance", "Leech Chance", "Leech_Chance", "chance", g_esAbility[type].g_flLeechChance, value, 0.0, 100.0);
		g_esAbility[type].g_iLeechDuration = iGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "LeechDuration", "Leech Duration", "Leech_Duration", "duration", g_esAbility[type].g_iLeechDuration, value, 1, 999999);
		g_esAbility[type].g_iLeechHit = iGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "LeechHit", "Leech Hit", "Leech_Hit", "hit", g_esAbility[type].g_iLeechHit, value, 0, 1);
		g_esAbility[type].g_iLeechHitMode = iGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "LeechHitMode", "Leech Hit Mode", "Leech_Hit_Mode", "hitmode", g_esAbility[type].g_iLeechHitMode, value, 0, 2);
		g_esAbility[type].g_flLeechInterval = flGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "LeechInterval", "Leech Interval", "Leech_Interval", "interval", g_esAbility[type].g_flLeechInterval, value, 0.1, 999999.0);
		g_esAbility[type].g_flLeechRange = flGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "LeechRange", "Leech Range", "Leech_Range", "range", g_esAbility[type].g_flLeechRange, value, 1.0, 999999.0);
		g_esAbility[type].g_flLeechRangeChance = flGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "LeechRangeChance", "Leech Range Chance", "Leech_Range_Chance", "rangechance", g_esAbility[type].g_flLeechRangeChance, value, 0.0, 100.0);
...
public void MT_OnAbilityActivated(int tank)
{
	...
	if (MT_IsTankSupported(tank) && (!MT_IsTankSupported(tank, MT_CHECK_FAKECLIENT) || g_esCache[tank].g_iHumanAbility != 1) && MT_IsCustomTankSupported(tank) && g_esCache[tank].g_iLeechAbility == 1 && g_esCache[tank].g_iComboAbility == 0)
	{
...
static void vLeechAbility(int tank, float random, int pos = -1)
{
	if (bIsAreaNarrow(tank, g_esCache[tank].g_flOpenAreasOnly) || MT_DoesTypeRequireHumans(g_esPlayer[tank].g_iTankType) || (g_esCache[tank].g_iRequiresHumans > 0 && iGetHumanCount() < g_esCache[tank].g_iRequiresHumans) || (!MT_HasAdminAccess(tank) && !bHasAdminAccess(tank, g_esAbility[g_esPlayer[tank].g_iTankType].g_iAccessFlags, g_esPlayer[tank].g_iAccessFlags)))
	{
		return;
	...
	if (!MT_IsTankSupported(tank, MT_CHECK_FAKECLIENT) || (g_esPlayer[tank].g_iAmmoCount < g_esCache[tank].g_iHumanAmmo && g_esCache[tank].g_iHumanAmmo > 0))
	{
		...
		flRange = (pos != -1) ? MT_GetCombinationSetting(tank, 8, pos) : g_esCache[tank].g_flLeechRange;
		flChance = (pos != -1) ? MT_GetCombinationSetting(tank, 9, pos) : g_esCache[tank].g_flLeechRangeChance;
		...
		for (int iSurvivor = 1; iSurvivor <= MaxClients; iSurvivor++)
		{
			if (bIsSurvivor(iSurvivor, MT_CHECK_INGAME|MT_CHECK_ALIVE) && !MT_IsAdminImmune(iSurvivor, tank) && !bIsAdminImmune(iSurvivor, g_esPlayer[tank].g_iTankType, g_esAbility[g_esPlayer[tank].g_iTankType].g_iImmunityFlags, g_esPlayer[iSurvivor].g_iImmunityFlags))
			{
				...
				{
					vLeechHit(iSurvivor, tank, random, flChance, g_esCache[tank].g_iLeechAbility, MT_MESSAGE_RANGE, MT_ATTACK_RANGE, pos);
...
static void vLeechHit(int survivor, int tank, float random, float chance, int enabled, int messages, int flags, int pos = -1)
{
		...
		if (!MT_IsTankSupported(tank, MT_CHECK_FAKECLIENT) || (g_esPlayer[tank].g_iAmmoCount < g_esCache[tank].g_iHumanAmmo && g_esCache[tank].g_iHumanAmmo > 0))
		{
			...
			if (random <= chance && !g_esPlayer[survivor].g_bAffected)
			{
				...
				if (MT_IsTankSupported(tank, MT_CHECK_FAKECLIENT) && g_esCache[tank].g_iHumanAbility == 1 && (flags & MT_ATTACK_RANGE) && (g_esPlayer[tank].g_iCooldown == -1 || g_esPlayer[tank].g_iCooldown < iTime))
				{
					...
					MT_PrintToChat(tank, "%s %t", MT_TAG3, "LeechHuman", g_esPlayer[tank].g_iAmmoCount, g_esCache[tank].g_iHumanAmmo);
				...
				flInterval = (pos != -1) ? MT_GetCombinationSetting(tank, 5, pos) : g_esCache[tank].g_flLeechInterval;
				DataPack dpLeech;
...
public Action tTimerCombo(Handle timer, DataPack pack)
{
	...
	if (!MT_IsCorePluginEnabled() || !MT_IsTankSupported(iTank) || (!MT_HasAdminAccess(iTank) && !bHasAdminAccess(iTank, g_esAbility[g_esPlayer[iTank].g_iTankType].g_iAccessFlags, g_esPlayer[iTank].g_iAccessFlags)) || !MT_IsTypeEnabled(g_esPlayer[iTank].g_iTankType) || !MT_IsCustomTankSupported(iTank) || g_esCache[iTank].g_iLeechAbility == 0)
	{
		return Plugin_Stop;
	}
...
public Action tTimerCombo2(Handle timer, DataPack pack)
{
	...
	int iTank = GetClientOfUserId(pack.ReadCell());
	if (!MT_IsCorePluginEnabled() || !MT_IsTankSupported(iTank) || (!MT_HasAdminAccess(iTank) && !bHasAdminAccess(iTank, g_esAbility[g_esPlayer[iTank].g_iTankType].g_iAccessFlags, g_esPlayer[iTank].g_iAccessFlags)) || !MT_IsTypeEnabled(g_esPlayer[iTank].g_iTankType) || !MT_IsCustomTankSupported(iTank) || g_esCache[iTank].g_iLeechHit == 0)
	{
		return Plugin_Stop;
	}
	...
	if ((g_esCache[iTank].g_iLeechHitMode == 0 || g_esCache[iTank].g_iLeechHitMode == 1) && (StrEqual(sClassname, "weapon_tank_claw") || StrEqual(sClassname, "tank_rock")))
	{
		vLeechHit(iSurvivor, iTank, flRandom, flChance, g_esCache[iTank].g_iLeechHit, MT_MESSAGE_MELEE, MT_ATTACK_CLAW, iPos);
	}
	else if ((g_esCache[iTank].g_iLeechHitMode == 0 || g_esCache[iTank].g_iLeechHitMode == 2) && StrEqual(sClassname, "weapon_melee"))
	{
		vLeechHit(iSurvivor, iTank, flRandom, flChance, g_esCache[iTank].g_iLeechHit, MT_MESSAGE_MELEE, MT_ATTACK_MELEE, iPos);
	}
...
public Action tTimerLeech(Handle timer, DataPack pack)
{
	...
	iPos = pack.ReadCell();
	iDuration = (iPos != -1) ? RoundToNearest(MT_GetCombinationSetting(iTank, 4, iPos)) : g_esCache[iTank].g_iLeechDuration;
	iTime = pack.ReadCell();
