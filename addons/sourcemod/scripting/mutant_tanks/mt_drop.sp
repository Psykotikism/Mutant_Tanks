...
public void MT_OnEventFired(Event event, const char[] name, bool dontBroadcast)
{
	...
	else if (StrEqual(name, "player_death"))
	{
		...
		if (MT_IsTankSupported(iTank, MT_CHECK_INDEX|MT_CHECK_INGAME|MT_CHECK_INKICKQUEUE) && MT_IsCustomTankSupported(iTank) && g_esPlayer[iTank].g_bActivated)
		{
...
static void vDropWeapon(int tank, int value, float random, int pos = -1)
{
	...
	if (MT_IsTankSupported(tank, MT_CHECK_INDEX|MT_CHECK_INGAME|MT_CHECK_INKICKQUEUE) && MT_IsCustomTankSupported(tank) && g_esCache[tank].g_iDropAbility == 1 && random <= flChance && bIsValidEntRef(g_esPlayer[tank].g_iWeapon))
	{
