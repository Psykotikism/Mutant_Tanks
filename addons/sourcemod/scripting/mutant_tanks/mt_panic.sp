...
#define MT_CONFIG_SECTION "panicability"
#define MT_CONFIG_SECTION2 "panic ability"
#define MT_CONFIG_SECTION3 "panic_ability"
#define MT_CONFIG_SECTION4 "panic"
...
public void MT_OnCombineAbilities(int tank, int type, float random, const char[] combo, int survivor, int weapon, const char[] classname)
{
	...
	{
		if (type == MT_COMBO_MAINRANGE && g_esCache[tank].g_iPanicAbility == 1 && g_esCache[tank].g_iComboAbility == 1 && !g_esPlayer[tank].g_bActivated)
		{
...
public void MT_OnConfigsLoaded(const char[] subsection, const char[] key, const char[] value, int type, int admin, int mode)
{
	if (mode == 3 && bIsValidClient(admin))
	{
		...
		g_esPlayer[admin].g_iPanicAbility = iGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "AbilityEnabled", "Ability Enabled", "Ability_Enabled", "aenabled", g_esPlayer[admin].g_iPanicAbility, value, 0, 3);
		g_esPlayer[admin].g_iPanicMessage = iGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "AbilityMessage", "Ability Message", "Ability_Message", "message", g_esPlayer[admin].g_iPanicMessage, value, 0, 1);
		g_esPlayer[admin].g_flPanicChance = flGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "PanicChance", "Panic Chance", "Panic_Chance", "chance", g_esPlayer[admin].g_flPanicChance, value, 0.0, 100.0);
		g_esPlayer[admin].g_flPanicInterval = flGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "PanicInterval", "Panic Interval", "Panic_Interval", "interval", g_esPlayer[admin].g_flPanicInterval, value, 0.1, 999999.0);
	...
	if (mode < 3 && type > 0)
	{
		...
		g_esAbility[type].g_iPanicAbility = iGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "AbilityEnabled", "Ability Enabled", "Ability_Enabled", "aenabled", g_esAbility[type].g_iPanicAbility, value, 0, 3);
		g_esAbility[type].g_iPanicMessage = iGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "AbilityMessage", "Ability Message", "Ability_Message", "message", g_esAbility[type].g_iPanicMessage, value, 0, 1);
		g_esAbility[type].g_flPanicChance = flGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "PanicChance", "Panic Chance", "Panic_Chance", "chance", g_esAbility[type].g_flPanicChance, value, 0.0, 100.0);
		g_esAbility[type].g_flPanicInterval = flGetKeyValue(subsection, MT_CONFIG_SECTIONS, key, "PanicInterval", "Panic Interval", "Panic_Interval", "interval", g_esAbility[type].g_flPanicInterval, value, 0.1, 999999.0);
...
public void MT_OnAbilityActivated(int tank)
{
	...
	if (MT_IsTankSupported(tank) && (!MT_IsTankSupported(tank, MT_CHECK_FAKECLIENT) || g_esCache[tank].g_iHumanAbility != 1) && MT_IsCustomTankSupported(tank) && g_esCache[tank].g_iPanicAbility == 1 && g_esCache[tank].g_iComboAbility == 0 && !g_esPlayer[tank].g_bActivated)
	{
		vPanicAbility(tank);
	}
}
...
static void vPanic(int tank, int pos = -1)
{
	g_esPlayer[tank].g_bActivated = true;
...
static void vPanic2(int tank, int pos = -1)
{
	if (bIsAreaNarrow(tank, g_esCache[tank].g_iOpenAreasOnly) || MT_DoesTypeRequireHumans(g_esPlayer[tank].g_iTankType) || (g_esCache[tank].g_iRequiresHumans > 0 && iGetHumanCount() < g_esCache[tank].g_iRequiresHumans) || (!MT_HasAdminAccess(tank) && !bHasAdminAccess(tank, g_esAbility[g_esPlayer[tank].g_iTankType].g_iAccessFlags, g_esPlayer[tank].g_iAccessFlags)))
	{
		return;
	...
	flInterval = (pos != -1) ? MT_GetCombinationSetting(tank, 5, pos) : g_esCache[tank].g_flPanicInterval;
	DataPack dpPanic;
...
public Action tTimerCombo(Handle timer, DataPack pack)
{
	...
	if (!MT_IsCorePluginEnabled() || !MT_IsTankSupported(iTank) || (!MT_HasAdminAccess(iTank) && !bHasAdminAccess(iTank, g_esAbility[g_esPlayer[iTank].g_iTankType].g_iAccessFlags, g_esPlayer[iTank].g_iAccessFlags)) || !MT_IsTypeEnabled(g_esPlayer[iTank].g_iTankType) || !MT_IsCustomTankSupported(iTank) || g_esCache[iTank].g_iPanicAbility == 0 || g_esPlayer[iTank].g_bActivated)
	{
		return Plugin_Stop;
	}
