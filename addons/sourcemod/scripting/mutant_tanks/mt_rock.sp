...
public void MT_OnConfigsLoaded(const char[] subsection, const char[] key, const char[] value, int type, int admin, int mode)
{
	if (mode == 3 && bIsValidClient(admin))
	{
		...
		g_esPlayer[admin].g_iRequiresHumans = iGetKeyValue(subsection, "rockability", "rock ability", "rock_ability", "rock", key, "RequiresHumans", "Requires Humans", "Requires_Humans", "hrequire", g_esPlayer[admin].g_iRequiresHumans, value, 0, 1);
	...
	if (mode < 3 && type > 0)
	{
		...
		g_esAbility[type].g_iRequiresHumans = iGetKeyValue(subsection, "rockability", "rock ability", "rock_ability", "rock", key, "RequiresHumans", "Requires Humans", "Requires_Humans", "hrequire", g_esAbility[type].g_iRequiresHumans, value, 0, 1);
...
static void vReset2(int tank)
{
	...
	if (g_esCache[tank].g_iRockMessage == 1)
	{
		...
		MT_LogMessage(MT_LOG_ABILITY, "%s %t", MT_TAG2, "Rock2", sTankName);
	}
}
	...
	if (!MT_IsTankSupported(tank, MT_CHECK_FAKECLIENT) || (g_esPlayer[tank].g_iCount < g_esCache[tank].g_iHumanAmmo && g_esCache[tank].g_iHumanAmmo > 0))
	{
		if (GetRandomFloat(0.1, 100.0) <= g_esCache[tank].g_flRockChance)
		{
			...
			if (g_esCache[tank].g_iRockMessage == 1)
			{
				static char sTankName[33];
				MT_GetTankName(tank, sTankName);
				MT_LogMessage(MT_LOG_ABILITY, "%s %t", MT_TAG2, "Rock", sTankName);
			}
		}
