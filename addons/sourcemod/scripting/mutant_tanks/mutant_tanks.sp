...
public void OnWeaponEquipPost(int client, int weapon)
{
	if (bIsSurvivor(client) && weapon > MaxClients)
	{
		...
		if (g_esGeneral.g_hSDKGetMaxClip1 != null && GetPlayerWeaponSlot(client, 0) == weapon)
		{
			if (!bDeveloper)
			{
				g_esPlayer[client].g_bFixedAmmo[0] = false;
			}
		...
		else if (g_esGeneral.g_hSDKGetMaxClip1 != null && GetPlayerWeaponSlot(client, 1) == weapon && (StrContains(sWeapon, "pistol") != -1 || StrEqual(sWeapon, "weapon_chainsaw")))
		{
			if (!bDeveloper)
			{
				g_esPlayer[client].g_bFixedAmmo[1] = false;
			}
...
{
	if (g_esGeneral.g_bPluginEnabled && bIsSurvivor(entity) && (bIsDeveloper(entity, 5) || bIsDeveloper(entity, 11) || (g_esPlayer[entity].g_iRewardTypes & MT_REWARD_SPEEDBOOST) || (g_esPlayer[entity].g_iRewardTypes & MT_REWARD_GODMODE)))
	{
		static float flOrigin[3];
							...
							for (int iPos = 0; iPos < sizeof(sSet); iPos++)
							{
								if (sSet[iPos][0] != '\0')
								{
									...
									for (int iPos2 = 0; iPos2 < sizeof(sValue); iPos2++)
									{
										if (sValue[iPos2][0] != '\0')
										{
											g_esPlayer[survivor].g_iScreenColorVisual[iPos2] = iGetRandomColor(StringToInt(sValue[iPos2]));
										}
									}
								}
							}
							...
							for (int iPos = 0; iPos < sizeof(sSet); iPos++)
							{
								if (sSet[iPos][0] != '\0')
								{
									...
									for (int iPos2 = 0; iPos2 < sizeof(sValue); iPos2++)
									{
										if (sValue[iPos2][0] != '\0')
										{
											g_esPlayer[survivor].g_iGlowColorVisual[iPos2] = iGetRandomColor(StringToInt(sValue[iPos2]));
										}
									}
								}
							}
							...
							for (int iPos = 0; iPos < sizeof(sSet); iPos++)
							{
								if (sSet[iPos][0] != '\0')
								{
									...
									for (int iPos2 = 0; iPos2 < sizeof(sValue); iPos2++)
									{
										if (sValue[iPos2][0] != '\0')
										{
											iColor[iPos2] = iGetRandomColor(StringToInt(sValue[iPos2]));
										}
									}
								}
							}
...
static int iGetMaxAmmo(int survivor, int type, int weapon, bool reserve, bool reset = false)
{
	if (weapon > MaxClients)
	{
		...
		bSurvivor = bIsSurvivor(survivor), bRewarded = bSurvivor ? (bIsDeveloper(survivor, 6) || (g_esPlayer[survivor].g_iRewardTypes & MT_REWARD_AMMO)) : false;
		static int iType;
