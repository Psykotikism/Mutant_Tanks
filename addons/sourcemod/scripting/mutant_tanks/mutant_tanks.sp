...
public any aNative_SetTankType(Handle plugin, int numParams)
{
	...
	if (bIsTank(iTank))
	{
		switch (bMode)
		{
			case true:
			{
				vSetColor(iTank, iType, .revert = (g_esPlayer[iTank].g_iTankType == iType));
				vTankSpawn(iTank, 5);
			}
			case false:
			{
...
public void OnAllPluginsLoaded()
{
	...
	if (gdMutantTanks != null)
	{
		...
		vSetupDetour(g_esGeneral.g_ddTankClawGroundPoundDetour, gdMutantTanks, "MTDetour_CTankClaw::GroundPound");
...
#endif
				vToggleEffects(param1, .type = param2, .toggle = false);
			}
			else
			{
				...
				g_esGeneral.g_ckMTPrefs.Set(param1, sValue);
#endif
				vToggleEffects(param1, .type = param2);
			}
		...
		else if (StrEqual(name, "player_no_longer_it"))
		{
			...
			else if (bIsSurvivor(iPlayer) && g_bSecondGame)
			{
				switch (bIsDeveloper(iPlayer, 0))
				{
					...
					case false: vToggleEffects(iPlayer, .type = 5);
				}
			}
	...
	if (bCanTypeSpawn())
	{
		...
		else if (type != -1)
		{
			switch (bVersus)
			{
				...
				{
					iType = (type > 0) ? type : g_esPlayer[tank].g_iTankType;
					vSetColor(tank, iType, false, .store = true);
				}
			}
		}
...
static int iChooseTank(int tank, int exclude, int min = -1, int max = -1, bool mutate = true)
{
	...
	if (iChosen > 0)
	{
		...
		if (iRealType > 0)
		{
			if (mutate)
			{
				vSetColor(tank, iRealType, false, .store = true);
			}
...
public Action tTimerRandomize(Handle timer, DataPack pack)
{
	...
	static int iType;
	iType = iChooseTank(iTank, 2, .mutate = false);
