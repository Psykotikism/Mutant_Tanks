...
public Action OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype)
{
	if (MT_IsCorePluginEnabled() && bIsValidClient(victim, MT_CHECK_INDEX|MT_CHECK_INGAME|MT_CHECK_ALIVE|MT_CHECK_INKICKQUEUE) && damage >= 0.5)
	{
		...
		if (MT_IsTankSupported(attacker) && bIsCloneAllowed(attacker) && (g_esCache[attacker].g_iChokeHitMode == 0 || g_esCache[attacker].g_iChokeHitMode == 1) && bIsSurvivor(victim))
		{
			...
			{
				vChokeHit(victim, attacker, g_esCache[attacker].g_flChokeChance, g_esCache[attacker].g_iChokeHit, MT_MESSAGE_MELEE, MT_ATTACK_CLAW);
			}
		}
		else if (MT_IsTankSupported(victim) && bIsCloneAllowed(victim) && (g_esCache[victim].g_iChokeHitMode == 0 || g_esCache[victim].g_iChokeHitMode == 2) && bIsSurvivor(attacker))
		{
			...
			{
				vChokeHit(attacker, victim, g_esCache[victim].g_flChokeChance, g_esCache[victim].g_iChokeHit, MT_MESSAGE_MELEE, MT_ATTACK_MELEE);
			}
		}
	}
...
public void MT_OnConfigsLoaded(const char[] subsection, const char[] key, const char[] value, int type, int admin, int mode)
{
	if (mode == 3 && bIsValidClient(admin))
	{
		...
		g_esPlayer[admin].g_iRequiresHumans = iGetKeyValue(subsection, "chokeability", "choke ability", "choke_ability", "choke", key, "RequiresHumans", "Requires Humans", "Requires_Humans", "hrequire", g_esPlayer[admin].g_iRequiresHumans, value, 0, 1);
	...
	if (mode < 3 && type > 0)
	{
		...
		g_esAbility[type].g_iRequiresHumans = iGetKeyValue(subsection, "chokeability", "choke ability", "choke_ability", "choke", key, "RequiresHumans", "Requires Humans", "Requires_Humans", "hrequire", g_esAbility[type].g_iRequiresHumans, value, 0, 1);
...
public void MT_OnAbilityActivated(int tank)
{
	...
	if (MT_IsTankSupported(tank) && (!MT_IsTankSupported(tank, MT_CHECK_FAKECLIENT) || g_esCache[tank].g_iHumanAbility != 1) && bIsCloneAllowed(tank) && g_esCache[tank].g_iChokeAbility == 1)
	{
		vChokeAbility(tank);
	}
}
	...
	if (!MT_IsTankSupported(tank, MT_CHECK_FAKECLIENT) || (g_esPlayer[tank].g_iCount < g_esCache[tank].g_iHumanAmmo && g_esCache[tank].g_iHumanAmmo > 0))
	{
		...
		for (int iSurvivor = 1; iSurvivor <= MaxClients; iSurvivor++)
		{
			if (bIsSurvivor(iSurvivor, MT_CHECK_INGAME|MT_CHECK_ALIVE|MT_CHECK_INKICKQUEUE) && !MT_IsAdminImmune(iSurvivor, tank) && !bIsAdminImmune(iSurvivor, g_esPlayer[tank].g_iTankType, g_esAbility[g_esPlayer[tank].g_iTankType].g_iImmunityFlags, g_esPlayer[iSurvivor].g_iImmunityFlags))
			{
				...
				if (flDistance <= g_esCache[tank].g_flChokeRange)
				{
					vChokeHit(iSurvivor, tank, g_esCache[tank].g_flChokeRangeChance, g_esCache[tank].g_iChokeAbility, MT_MESSAGE_RANGE, MT_ATTACK_RANGE);
		...
		if (!MT_IsTankSupported(tank, MT_CHECK_FAKECLIENT) || (g_esPlayer[tank].g_iCount < g_esCache[tank].g_iHumanAmmo && g_esCache[tank].g_iHumanAmmo > 0))
		{
			...
			if (GetRandomFloat(0.1, 100.0) <= chance && !g_esPlayer[survivor].g_bAffected)
			{
				...
				if (MT_IsTankSupported(tank, MT_CHECK_FAKECLIENT) && g_esCache[tank].g_iHumanAbility == 1 && (flags & MT_ATTACK_RANGE) && (g_esPlayer[tank].g_iCooldown == -1 || g_esPlayer[tank].g_iCooldown < iTime))
				{
					...
					MT_PrintToChat(tank, "%s %t", MT_TAG3, "ChokeHuman", g_esPlayer[tank].g_iCount, g_esCache[tank].g_iHumanAmmo);
				...
				CreateDataTimer(g_esCache[tank].g_flChokeDelay, tTimerChokeLaunch, dpChokeLaunch, TIMER_FLAG_NO_MAPCHANGE);
...
public Action tTimerChokeLaunch(Handle timer, DataPack pack)
{
	...
	static int iTank, iType, iChokeEnabled;
	iTank = GetClientOfUserId(pack.ReadCell());
	iType = pack.ReadCell();
	iChokeEnabled = pack.ReadCell();
	if (!MT_IsTankSupported(iTank) || (!MT_HasAdminAccess(iTank) && !bHasAdminAccess(iTank, g_esAbility[g_esPlayer[iTank].g_iTankType].g_iAccessFlags, g_esPlayer[iTank].g_iAccessFlags)) || !MT_IsTypeEnabled(g_esPlayer[iTank].g_iTankType) || !bIsCloneAllowed(iTank) || iType != g_esPlayer[iTank].g_iTankType || MT_IsAdminImmune(iSurvivor, iTank) || bIsAdminImmune(iSurvivor, g_esPlayer[iTank].g_iTankType, g_esAbility[g_esPlayer[iTank].g_iTankType].g_iImmunityFlags, g_esPlayer[iSurvivor].g_iImmunityFlags) || iChokeEnabled == 0)
	{
		g_esPlayer[iSurvivor].g_bAffected = false;
		g_esPlayer[iSurvivor].g_iOwner = 0;
...
public Action tTimerChokeDamage(Handle timer, DataPack pack)
{
	...
	static int iChokeEnabled, iTime;
	iChokeEnabled = pack.ReadCell();
	iTime = pack.ReadCell();
	if (iChokeEnabled == 0 || (iTime + g_esCache[iTank].g_iChokeDuration) < GetTime())
	{
		vReset2(iSurvivor, iTank, iMessage);
