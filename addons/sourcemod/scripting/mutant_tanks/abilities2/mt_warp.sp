	...
	switch (action)
	{
		...
		case MenuAction_Select:
		{
			switch (param2)
			{
				...
				case 3:
				{
					switch (g_esWarpCache[param1].g_iHumanMode)
					{
...
#endif
{
	if ((mode == -1 || mode == 3) && bIsValidClient(admin))
	{
		if (special && specsection[0] != '\0')
		{
			...
			g_esWarpTeammate[admin].g_iWarpRockBreak = iGetKeyValue(subsection, MT_WARP_SECTION, MT_WARP_SECTION2, MT_WARP_SECTION3, MT_WARP_SECTION4, key, "WarpPin", "Warp Pin", "Warp_Pin", "pin", g_esWarpTeammate[admin].g_iWarpRockBreak, value, -1, 1);
			g_esWarpTeammate[admin].g_flWarpRockChance = flGetKeyValue(subsection, MT_WARP_SECTION, MT_WARP_SECTION2, MT_WARP_SECTION3, MT_WARP_SECTION4, key, "WarpPinChance", "Warp Pin Chance", "Warp_Pin_Chance", "pinchance", g_esWarpTeammate[admin].g_flWarpRockChance, value, -1.0, 100.0);
	...
	if (mode < 3 && type > 0)
	{
		if (special && specsection[0] != '\0')
		{
			...
			g_esWarpSpecial[type].g_iWarpRockBreak = iGetKeyValue(subsection, MT_WARP_SECTION, MT_WARP_SECTION2, MT_WARP_SECTION3, MT_WARP_SECTION4, key, "WarpPin", "Warp Pin", "Warp_Pin", "pin", g_esWarpSpecial[type].g_iWarpRockBreak, value, -1, 1);
			g_esWarpSpecial[type].g_flWarpRockChance = flGetKeyValue(subsection, MT_WARP_SECTION, MT_WARP_SECTION2, MT_WARP_SECTION3, MT_WARP_SECTION4, key, "WarpPinChance", "Warp Pin Chance", "Warp_Pin_Chance", "pinchance", g_esWarpSpecial[type].g_flWarpRockChance, value, -1.0, 100.0);
...
#endif
{
	if (MT_IsTankSupported(tank, MT_CHECK_INDEX|MT_CHECK_INGAME|MT_CHECK_ALIVE|MT_CHECK_FAKECLIENT) && MT_IsCustomTankSupported(tank))
	{
		...
		if ((button & MT_MAIN_KEY) && (g_esWarpCache[tank].g_iWarpAbility == 2 || g_esWarpCache[tank].g_iWarpAbility == 3) && g_esWarpCache[tank].g_iHumanAbility == 1)
		{
			...
			int iHumanMode = g_esWarpCache[tank].g_iHumanMode;
			...
			switch (iHumanMode)
			{
				...
				case 1, 2:
				{
					if ((iHumanMode == 2 && g_esWarpPlayer[tank].g_bActivated) || (g_esWarpPlayer[tank].g_iAmmoCount < g_esWarpCache[tank].g_iHumanAmmo && g_esWarpCache[tank].g_iHumanAmmo > 0))
					{
						if (!g_esWarpPlayer[tank].g_bActivated && !bRecharging)
						{
						...
						else if (g_esWarpPlayer[tank].g_bActivated)
						{
							switch (iHumanMode)
							{
								case 1: MT_PrintToChat(tank, "%s %t", MT_TAG3, "WarpHuman4");
								case 2: vWarpReset2(tank);
							}
						}
						else if (bRecharging)
						{
