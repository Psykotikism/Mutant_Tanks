...
float g_flFragileBulletMultiplier[MT_MAXTYPES + 1], g_flFragileChance[MT_MAXTYPES + 1], g_flFragileDuration[MT_MAXTYPES + 1], g_flFragileExplosiveMultiplier[MT_MAXTYPES + 1], g_flFragileFireMultiplier[MT_MAXTYPES + 1], g_flFragileMeleeMultiplier[MT_MAXTYPES + 1], g_flHumanCooldown[MT_MAXTYPES + 1];

int g_iAccessFlags[MT_MAXTYPES + 1], g_iAccessFlags2[MAXPLAYERS + 1], g_iFragileAbility[MT_MAXTYPES + 1], g_iFragileCount[MAXPLAYERS + 1], g_iFragileMessage[MT_MAXTYPES + 1], g_iHumanAbility[MT_MAXTYPES + 1], g_iHumanAmmo[MT_MAXTYPES + 1], g_iHumanMode[MT_MAXTYPES + 1], g_iImmunityFlags[MT_MAXTYPES + 1], g_iImmunityFlags2[MAXPLAYERS + 1];
...
public int iFragileMenuHandler(Menu menu, MenuAction action, int param1, int param2)
{
	switch (action)
	{
		...
		case MenuAction_Select:
		{
			switch (param2)
			{
				case 0: MT_PrintToChat(param1, "%s %t", MT_TAG3, g_iFragileAbility[MT_GetTankType(param1)] == 0 ? "AbilityStatus1" : "AbilityStatus2");
				case 1: MT_PrintToChat(param1, "%s %t", MT_TAG3, "AbilityAmmo", g_iHumanAmmo[MT_GetTankType(param1)] - g_iFragileCount[param1], g_iHumanAmmo[MT_GetTankType(param1)]);
				...
				case 6: MT_PrintToChat(param1, "%s %t", MT_TAG3, "AbilityDuration", g_flFragileDuration[MT_GetTankType(param1)]);
...
public Action OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype)
{
	...
	{
		if (MT_IsTankSupported(victim) && bIsCloneAllowed(victim, g_bCloneInstalled) && g_bFragile[victim] && bIsSurvivor(attacker))
		{
			...
			{
				damage *= g_flFragileBulletMultiplier[MT_GetTankType(victim)];
			}
			else if (damagetype & DMG_BLAST || damagetype & DMG_BLAST_SURFACE || damagetype & DMG_AIRBOAT || damagetype & DMG_PLASMA)
			{
				damage *= g_flFragileExplosiveMultiplier[MT_GetTankType(victim)];
			}
			else if (damagetype & DMG_BURN)
			{
				damage *= g_flFragileFireMultiplier[MT_GetTankType(victim)];
			}
			else if (damagetype & DMG_SLASH || damagetype & DMG_CLUB)
			{
				damage *= g_flFragileMeleeMultiplier[MT_GetTankType(victim)];
			}
...
public void MT_OnConfigsLoaded(const char[] subsection, const char[] key, const char[] value, int type, int admin, int mode)
{
	...
	if (mode < 3 && type > 0)
	{
		...
		g_iHumanAmmo[type] = iGetValue(subsection, "fragileability", "fragile ability", "fragile_ability", "fragile", key, "HumanAmmo", "Human Ammo", "Human_Ammo", "hammo", g_iHumanAmmo[type], value, 0, 999999);
		g_flHumanCooldown[type] = flGetValue(subsection, "fragileability", "fragile ability", "fragile_ability", "fragile", key, "HumanCooldown", "Human Cooldown", "Human_Cooldown", "hcooldown", g_flHumanCooldown[type], value, 0.0, 999999.0);
		...
		g_flFragileBulletMultiplier[type] = flGetValue(subsection, "fragileability", "fragile ability", "fragile_ability", "fragile", key, "FragileBulletMultiplier", "Fragile Bullet Multiplier", "Fragile_Bullet_Multiplier", "bullet", g_flFragileBulletMultiplier[type], value, 1.0, 999999.0);
		...
		g_flFragileDuration[type] = flGetValue(subsection, "fragileability", "fragile ability", "fragile_ability", "fragile", key, "FragileDuration", "Fragile Duration", "Fragile_Duration", "duration", g_flFragileDuration[type], value, 0.1, 999999.0);
		g_flFragileExplosiveMultiplier[type] = flGetValue(subsection, "fragileability", "fragile ability", "fragile_ability", "fragile", key, "FragileExplosiveMultiplier", "Fragile Explosive Multiplier", "Fragile_Explosive_Multiplier", "explosive", g_flFragileExplosiveMultiplier[type], value, 1.0, 999999.0);
		g_flFragileFireMultiplier[type] = flGetValue(subsection, "fragileability", "fragile ability", "fragile_ability", "fragile", key, "FragileFireMultiplier", "Fragile Fire Multiplier", "Fragile_Fire_Multiplier", "fire", g_flFragileFireMultiplier[type], value, 1.0, 999999.0);
		g_flFragileMeleeMultiplier[type] = flGetValue(subsection, "fragileability", "fragile ability", "fragile_ability", "fragile", key, "FragileMeleeMultiplier", "Fragile Melee Multiplier", "Fragile_Melee_Multiplier", "melee", g_flFragileMeleeMultiplier[type], value, 1.0, 999999.0);
...
public void MT_OnAbilityActivated(int tank)
{
	...
	if (MT_IsTankSupported(tank) && (!MT_IsTankSupported(tank, MT_CHECK_FAKECLIENT) || g_iHumanAbility[MT_GetTankType(tank)] == 0) && bIsCloneAllowed(tank, g_bCloneInstalled) && g_iFragileAbility[MT_GetTankType(tank)] == 1 && !g_bFragile[tank])
	{
...
public void MT_OnButtonPressed(int tank, int button)
{
	if (MT_IsTankSupported(tank, MT_CHECK_INDEX|MT_CHECK_INGAME|MT_CHECK_ALIVE|MT_CHECK_INKICKQUEUE|MT_CHECK_FAKECLIENT) && bIsCloneAllowed(tank, g_bCloneInstalled))
	{
		...
		{
			if (g_iFragileAbility[MT_GetTankType(tank)] == 1 && g_iHumanAbility[MT_GetTankType(tank)] == 1)
			{
				switch (g_iHumanMode[MT_GetTankType(tank)])
				{
					...
					case 1:
					{
						if (g_iFragileCount[tank] < g_iHumanAmmo[MT_GetTankType(tank)] && g_iHumanAmmo[MT_GetTankType(tank)] > 0)
						{
							if (!g_bFragile[tank] && !g_bFragile2[tank])
							{
								...
								MT_PrintToChat(tank, "%s %t", MT_TAG3, "FragileHuman", g_iFragileCount[tank], g_iHumanAmmo[MT_GetTankType(tank)]);
							}
						}
						else
						{
...
public void MT_OnButtonReleased(int tank, int button)
{
	if (MT_IsTankSupported(tank, MT_CHECK_INDEX|MT_CHECK_INGAME|MT_CHECK_ALIVE|MT_CHECK_INKICKQUEUE|MT_CHECK_FAKECLIENT) && bIsCloneAllowed(tank, g_bCloneInstalled))
	{
		if (button & MT_MAIN_KEY == MT_MAIN_KEY)
		{
			if (g_iFragileAbility[MT_GetTankType(tank)] == 1 && g_iHumanAbility[MT_GetTankType(tank)] == 1)
			{
				if (g_iHumanMode[MT_GetTankType(tank)] == 1 && g_bFragile[tank] && !g_bFragile2[tank])
				{
...
static void vFragileAbility(int tank)
{
	...
	if (g_iFragileCount[tank] < g_iHumanAmmo[MT_GetTankType(tank)] && g_iHumanAmmo[MT_GetTankType(tank)] > 0)
	{
		if (GetRandomFloat(0.1, 100.0) <= g_flFragileChance[MT_GetTankType(tank)])
		{
			g_bFragile[tank] = true;

			CreateTimer(g_flFragileDuration[MT_GetTankType(tank)], tTimerStopFragile, GetClientUserId(tank), TIMER_FLAG_NO_MAPCHANGE);
			...
			if (g_iFragileMessage[MT_GetTankType(tank)] == 1)
			{
				char sTankName[33];
...
public Action tTimerStopFragile(Handle timer, int userid)
{
	...
	if (!MT_IsTankSupported(iTank) || !bIsCloneAllowed(iTank, g_bCloneInstalled) || !g_bFragile[iTank])
	{
	...
	if (MT_IsTankSupported(iTank, MT_CHECK_FAKECLIENT) && (MT_HasAdminAccess(iTank) || bHasAdminAccess(iTank)) && g_iHumanAbility[MT_GetTankType(iTank)] == 1 && !g_bFragile2[iTank])
	{
		vReset2(iTank);
	}

	if (g_iFragileMessage[MT_GetTankType(iTank)] == 1)
	{
		char sTankName[33];
...
{
	int iTank = GetClientOfUserId(userid);
	if (!MT_IsTankSupported(iTank, MT_CHECK_INDEX|MT_CHECK_INGAME|MT_CHECK_ALIVE|MT_CHECK_INKICKQUEUE|MT_CHECK_FAKECLIENT) || !bIsCloneAllowed(iTank, g_bCloneInstalled) || !g_bFragile2[iTank])
	{
